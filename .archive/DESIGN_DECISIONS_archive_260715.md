# Archive: AIHack Design Decisions historical full snapshot

- Archived: 2026-07-15
- Source: [DESIGN_DECISIONS.md](../DESIGN_DECISIONS.md)
- Previous archive: none (first archive)
- Policy: immutable snapshot; do not edit, delete, or rename
...
ADR-0001..ADR-0020은 과거 구현 결정과 이력이다. 아래 ADR은 v0.3.0 target에 대해 우선하며, 기존 결정과 충돌하면 아래 결정과 `spec.md`를 적용한다. Accepted는 계획 승인을 뜻하며 구현 완료를 뜻하지 않는다.
...
사용자의 제품 목표는 NetHack 3.6.7을 Rust로 재구성하고 local LLM을 메시지와 판정에 사용하는 것이다. ADR-0005의 “NetHack-inspired 독립 게임”은 이 목표를 충분히 표현하지 않으며, 줄 단위 포트는 기존 legacy 구조 문제와 출처 위험을 되살린다.
...
기본 endpoint는 loopback OpenAI-compatible HTTP이며 기본 enabled는 false다. connect 500ms, narrative 2000ms, decision 1500ms를 transport가 강제한다. request는 request ID와 `SessionRevision { turn, snapshot_hash }`를 포함한다. narrative와 soft verdict는 presentation-only다. suggestion은 current `ActionSpace`와 revision을 재검증하고 사용자의 `Y` 승인 뒤 normal submit path를 사용한다.
