...
| G-BUILD-006 | P0 | dependency exception lifecycle/graph drift | report 25의 parsed TOML/exact trigger와 cargo-deny 0.19.4는 부분 Verified, report 26에서 미래 approval 허용 재현 | report 26 시정 | SC-BUILD-01, SC-LICENSE-01 | Reopened |
...
| G-TEST-003 | P0 | R9 causal witness 원인별 독립성 | production gold/no-gold pair는 부분 Verified, report 26에서 summary label 삭제 negative의 tautology 재현 | report 26 시정 | SC-CAUSE-05, SC-CAUSE-06, SC-CAUSE-07 | Reopened |
...
| G-DOC-004 | P0 | 과거 finding과 현재 권한 상태 혼재 | report 26이 stale spec status와 terminal evidence 표현을 재개방 | report 26 시정 | SC-DOC-01 | Reopened |
...
| G-SEC-001 | P0 | artifact link/root/ambient path 경계 | runtime capability 경계는 부분 Verified, report 26에서 Windows trailing alias와 release root/hardlink 재현 | report 26 시정 | SEC-F001, FIN-F004, FIN-F014 | Reopened |
...
| G-FINAL-001 | P0 | final multi-audit FIN-F001..F018 및 report 26 독립 재감사 HOLD | report 25 RED/GREEN과 SHA `b732c42d` CI는 부분 증거, report 26 Major 8/Minor 4 시정 진행 | report 26 remediation | report 26 finding 12건과 FIN-F001..F018 | Reopened |
...
R0 문서화와 R1~R8의 기존 remediation은 `docs/audit/audit_report_21.md`가 report 20의 `IMP-F016`/`DBG-F008`을 Verified하면서 종결됐다. R6는 `audit_report_11.md`, R8 문서 closure는 `audit_report_21.md`가 종결 권한이다. report 23/24는 implementation SHA `2519bc8e0ede81c39f46b5778e62a41d4ca66901`와 Actions run `32107862171`로 historical closed다. report 25 시정 SHA `b732c42d62f295f4d8be64480c1d0a5a440fe738`와 Actions `32650404618`은 유효한 부분 evidence다. 현재 authority는 `docs/audit/audit_report_26.md`이며 Major 8/Minor 4의 표적 GREEN 뒤 전체 local gate와 새 clean same-SHA Ubuntu/Windows actual bundle을 통과할 때까지 관련 child/aggregate gap과 program/publication을 `Reopened`/HOLD로 유지한다.
