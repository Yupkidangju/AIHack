...
| G-BUILD-006 | P0 | `winx 0.36.4` exception lifecycle/graph drift | parsed TOML/exact trigger gate와 cargo-deny 0.19.4; SHA `b732c42d` Actions `32650404618` 양 OS success | report 25 시정 | SC-BUILD-01, SC-LICENSE-01 | Verified |
...
| G-TEST-003 | P0 | R9 causal witness 원인별 독립성 | production gold/no-gold pair, 독립 negative matrix와 seed 42/7/1234 x 1000턴 x 3회 record/hash; Actions `32650404618` 양 OS success | report 25 시정 | SC-CAUSE-05, SC-CAUSE-06, SC-CAUSE-07 | Verified |
...
| G-DOC-004 | P0 | 과거 finding과 현재 권한 상태 혼재 | report 23/24 historical closed, final report 1 partial evidence, report 25 current authority와 lifecycle gate; Actions `32650404618` | report 25 시정 | SC-DOC-01 | Verified |
...
| G-SEC-001 | P0 | artifact link/root/ambient path 경계 | normalized/file-identity guard, root no-follow, capped save, atomic replay와 Unix syncable parent descriptor; Actions `32650404618` 양 OS success | report 25 시정 | SEC-F001 | Verified |
...
| G-FINAL-001 | P0 | final multi-audit FIN-F001..F018 및 report 25 재현 HOLD | report 25 RED/GREEN, 전체 local gate와 SHA `b732c42d` Actions `32650404618` 양 OS success; 독립 재감사 pending | report 25 remediation | FIN-F001..F018과 report 25 finding 13건 | Verified |
...
R0 문서화와 R1~R8의 기존 remediation은 `docs/audit/audit_report_21.md`가 report 20의 `IMP-F016`/`DBG-F008`을 Verified하면서 종결됐다. R6는 `audit_report_11.md`, R8 문서 closure는 `audit_report_21.md`가 종결 권한이다. report 23/24는 implementation SHA `2519bc8e0ede81c39f46b5778e62a41d4ca66901`와 Actions run `32107862171`로 historical closed다. final multi-audit report 1의 첫 remediation은 `docs/audit/audit_report_25.md`가 9 Major/4 Minor production 결함을 재현해 partial evidence로 강등했다. report 25 시정은 SHA `b732c42d62f295f4d8be64480c1d0a5a440fe738`의 Actions `32650404618`에서 clean same-SHA 양 OS gate를 PASS해 child/aggregate gap을 함께 `Verified`로 올렸다. 현재 authority는 report 25이며 독립 재감사 뒤에만 `Closed`로 올린다.
