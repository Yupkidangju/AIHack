...
- 현재 권위는 `docs/audit/audit_report_30.md`다. ADR-0040 authority/public-surface 구현과 표적 GREEN은 완료했고 전체 local gate와 새 clean same-SHA Ubuntu/Windows CI가 pending이다. 후속 독립 PASS와 별도 게시 승인 전에는 program/외부 게시 HOLD다.
...
- The current authority is `docs/audit/audit_report_30.md`. ADR-0040 authority/public-surface implementation and targeted regressions pass; the full local gate and a new clean same-SHA Ubuntu/Windows CI run are pending. Program PASS and publication remain blocked pending an independent PASS and separate publication approval.
...
- 現在の権威は `docs/audit/audit_report_30.md` です。ADR-0040 authority/public-surface 実装と対象 regression は PASS し、全 local gate と新しい clean same-SHA Ubuntu/Windows CI は pending です。独立 PASS と別途公開承認までは program PASS と外部公開を保留します。
...
- 目前權威是 `docs/audit/audit_report_30.md`。ADR-0040 authority/public-surface 實作與目標 regression 已通過，完整 local gate 與新的 clean same-SHA Ubuntu/Windows CI 尚待完成。獨立 PASS 與另行發布核准前，program PASS 與對外發布維持暫停。
...
- 当前权威是 `docs/audit/audit_report_30.md`。ADR-0040 authority/public-surface 实现与目标 regression 已通过，完整 local gate 与新的 clean same-SHA Ubuntu/Windows CI 尚待完成。独立 PASS 与另行发布批准前，program PASS 和对外发布保持暂停。
