...
- 현재 권위는 `docs/audit/audit_report_28.md`다. report 27 predecessor `ea7822a5`/[Actions `32683076204`](https://github.com/Yupkidangju/AIHack/actions/runs/32683076204)는 유효하며, ADR-0038의 allocator/registry/equipment/TUI/archive/calendar 표적 회귀는 GREEN이고 전체 local gate와 새 clean same-SHA 양 OS CI를 진행 중이다. 새 독립 PASS와 별도 게시 승인 전에는 program/외부 게시 HOLD다.
...
- The current authority is `docs/audit/audit_report_28.md`. Report 27 predecessor `ea7822a5`/[Actions `32683076204`](https://github.com/Yupkidangju/AIHack/actions/runs/32683076204) remains valid; ADR-0038 allocator/registry/equipment/TUI/archive/calendar regressions are green, with the full local gate and new clean same-SHA two-OS CI in progress. Program PASS and publication remain blocked pending a new independent PASS and separate publication approval.
...
- 現在の権威は `docs/audit/audit_report_28.md` です。report 27 predecessor `ea7822a5`/Actions `32683076204` は有効で、ADR-0038 の allocator/registry/equipment/TUI/archive/calendar 対象回帰は GREEN、全 local gate と新しい clean same-SHA 両 OS CI は進行中です。新しい独立 PASS と別途公開承認までは program PASS と外部公開を保留します。
...
- 目前權威是 `docs/audit/audit_report_28.md`。report 27 predecessor `ea7822a5`/Actions `32683076204` 仍有效；ADR-0038 的 allocator/registry/equipment/TUI/archive/calendar 目標回歸已為 GREEN，完整 local gate 與新的 clean same-SHA 雙系統 CI 正在進行。新的獨立 PASS 與另行發布核准前，program PASS 與對外發布維持暫停。
...
- 当前权威是 `docs/audit/audit_report_28.md`。report 27 predecessor `ea7822a5`/Actions `32683076204` 仍有效；ADR-0038 的 allocator/registry/equipment/TUI/archive/calendar 目标回归已为 GREEN，完整 local gate 与新的 clean same-SHA 双系统 CI 正在进行。新的独立 PASS 与另行发布批准前，program PASS 和对外发布保持暂停。
