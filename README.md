...
- 현재 권위는 `docs/audit/audit_report_27.md`다. ADR-0037 시정 SHA `ea7822a5`의 [Actions `32683076204`](https://github.com/Yupkidangju/AIHack/actions/runs/32683076204)는 전체 local gate와 clean same-SHA Ubuntu/Windows actual bundle을 PASS했다. 새 독립 PASS와 별도 게시 승인 전에는 program/외부 게시 HOLD다.
...
- The current authority is `docs/audit/audit_report_27.md`. Remediation SHA `ea7822a5` passed the full local gate and clean same-SHA Ubuntu/Windows actual bundles in [Actions `32683076204`](https://github.com/Yupkidangju/AIHack/actions/runs/32683076204). Program PASS and publication remain blocked pending a new independent PASS and separate publication approval.
...
- 現在の権威は `docs/audit/audit_report_27.md` です。修正 SHA `ea7822a5` の Actions `32683076204` は全 local gate と clean same-SHA Ubuntu/Windows 実 bundle を PASS しました。新しい独立 PASS と別途公開承認までは program PASS と外部公開を保留します。
...
- 目前權威是 `docs/audit/audit_report_27.md`。修正 SHA `ea7822a5` 的 Actions `32683076204` 已通過完整 local gate 與 clean same-SHA Ubuntu/Windows 實際 bundle；新的獨立 PASS 與另行發布核准前，program PASS 與對外發布維持暫停。
...
- 当前权威是 `docs/audit/audit_report_27.md`。修正 SHA `ea7822a5` 的 Actions `32683076204` 已通过完整 local gate 与 clean same-SHA Ubuntu/Windows 实际 bundle；新的独立 PASS 与另行发布批准前，program PASS 和对外发布保持暂停。
