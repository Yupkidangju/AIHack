...
if "!BUILD_TYPE!"=="release" (
    set "STAGING_DIR="
    for /f "usebackq delims=" %%G in (`powershell -NoProfile -ExecutionPolicy Bypass -File "scripts\release_staging.ps1" -Mode New -Root "%CD%" -OutputDir "%OUTPUT_DIR%"`) do set "STAGING_DIR=%%G"
    if not defined STAGING_DIR goto build_fail
    set "PACKAGE_DIR=!STAGING_DIR!"
) else (
    if not exist "%OUTPUT_DIR%" mkdir "%OUTPUT_DIR%"
    if errorlevel 1 goto build_fail
    set "PACKAGE_DIR=%OUTPUT_DIR%"
)

git show HEAD:LICENSE > "!PACKAGE_DIR!\LICENSE"
if errorlevel 1 goto build_fail
copy /y "NOTICE" "!PACKAGE_DIR!\NOTICE" >nul
if errorlevel 1 goto build_fail
copy /y "MODIFICATIONS.md" "!PACKAGE_DIR!\MODIFICATIONS.md" >nul
if errorlevel 1 goto build_fail
copy /y "PROJECT_OWNER_LICENSE_APPROVAL.md" "!PACKAGE_DIR!\PROJECT_OWNER_LICENSE_APPROVAL.md" >nul
if errorlevel 1 goto build_fail
...
for %%F in (aihack.exe aihack-headless.exe) do (
    ...
    copy /y "!SOURCE_DIR!\%%F" "!PACKAGE_DIR!\%%F" >nul
    if errorlevel 1 goto build_fail
    if not exist "!PACKAGE_DIR!\%%F" (
        echo artifact 검증에 실패했습니다: !PACKAGE_DIR!\%%F
        goto build_fail
    )
)
...
if "!BUILD_TYPE!"=="release" (
    ...
    for /f "delims=" %%G in ('git show -s --format^=%%cs HEAD') do set "CANDIDATE_DATE=%%G"
    >"!PACKAGE_DIR!\RELEASE-METADATA" echo product=AIHack
    >>"!PACKAGE_DIR!\RELEASE-METADATA" echo version=0.3.0
    ...
    if errorlevel 1 goto build_fail
    if not exist "!PACKAGE_DIR!\aihack-0.3.0-source.zip" (
        echo 대응 소스 archive 검증에 실패했습니다: !PACKAGE_DIR!\aihack-0.3.0-source.zip
        goto build_fail
    )
...
if defined STAGING_DIR (
    powershell -NoProfile -ExecutionPolicy Bypass -File "scripts\release_staging.ps1" -Mode Cleanup -Root "%CD%" -OutputDir "%OUTPUT_DIR%" -Stage "!STAGING_DIR!" >nul 2>nul
)
endlocal
exit /b 1
