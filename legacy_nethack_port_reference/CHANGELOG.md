...
## [2.44.2] - 2026-04-02
### Fixed
- **[S6' EC-8] item_use 시스템 AccessDenied 패닉 (Save/Use)**: `Player` 및 `Equipment` 컴포넌트의 write_component 접근 권한 부재로 인한 Legion 런타임 패닉 디버그 및 수정
- **[S6' EC-9] Save 시 unknown component 및 key must be string 에러 완벽 해결**:
    - 누락된 15개 런타임 컴포넌트를 Registry에 일괄 등록 (MonsterState, ContainerTag 등)
    - `serde_json`의 HashMap 문자열 키 강제 제약 극복을 위해 `Inventory`, `Equipment`, `SpellKnowledge`, `Player`, `Dungeon`, `Grid` 객체들에 Custom Serialize/Deserialize 구현 적용 (내부 배열로 변환 직렬화)
- **[S6' EC-10] 맵 생성 시 고립된 공간(길 없음) 문제 수정**: 
    - `dodoor()`가 문 상한(방당 최대 4) 혹은 인접 조건으로 인해 문 생성을 거부했을 경우, 뚫어야 할 벽(코너 포함)이 그대로 남아 복도(Corridor) 생성을 차단하던 문제를 해결. 방 상실 방지를 위해 문 생성을 반려할 시 해당 벽을 강제로 복도 타일(`TileType::Corr`)로 개방하도록 처리하여 원본 넷핵처럼 맵간 통행을 보장함.
- **[S6' EC-11] 몬스터 간 겹침 및 플레이어 위치 오버랩(Overlap) 오류 수정**:
    - AI 시스템(`src/core/systems/ai/core.rs`) 내에서 움직임(`move_towards`, `move_random`) 판정 시, 한 턴에 여러 몬스터가 움직일 때 점유 데이터(`occupancy`)가 실시간으로 갱신되지 않아 여러 객체가 동일한 목표 타일을 향해 이동, 겹치는 문제를 해결.
    - AI가 플레이어 타일을 비어있는 공간으로 인식해 경로 마지막 위치로 밟아버리던 현상을 막기 위해, 초기 점유 리스트에 플레이어의 위치를 `insert`하고 `move_towards` 시 도착 타일 점유 여부를 검증하게 함.

### Changed
- **[S6' C-7] 직업(Role)별 초기 아이템(Starting Items) 네트핵 고증 및 차별화 적용**:
    - 모든 직업이 "long sword, small shield" (혹은 힐링 포션)만 들고 시작하던 부분을 삭제하고, `src/app.rs` 내에서 `Role` 열거형에 맞춰 각 직업별 고유 아이템(예: Ranger의 elven cloak과 dagger, Tourist의 dart, Wizard의 Scroll 등)을 세분화하여 지급하도록 변경.
    - 투사체(dart, arrow, shuriken)나 소모성 자원(tin) 스폰 시에는 난수로 20~40개씩 다량 지급되도록 로직 보강.

## [2.44.1] - 2026-04-02
### Fixed
- **[S6' EC-1] restart_game() 리소스 완전성**: 누락된 리소스 7개(AssetManager, Options, VisionSystem, DefaultInteractionProvider, PendingAltarUpdate, TeleportAction, StatusFlags) 추가. GameOver→재시작 시 패닉 방지
- **[S6' EC-2] SaveManager::load() 리소스 보완**: Load 후 누락되는 런타임 리소스 6개(ActionQueue, DeathResults, StatusFlags, TeleportAction, PendingAltarUpdate, DefaultInteractionProvider) 추가. Save→Quit→Load 시 시스템 패닉 방지
- **[S6' EC-3] restart_game() game_initialized 리셋**: GameOver→"Back to Title" 후 game_initialized가 true로 남아 Title→Continue에서 Player 없이 Playing 진입하는 문제 해결
- **[S6' EC-5] Load 분기 AssetManager/Options 주입**: Save Load 경로에서 Resources에 AssetManager와 Options를 재주입. 시스템의 리소스 참조 실패 방지
- **[S6' EC-6] restart_game() char_name_buf 초기화**: 이전 게임 캐릭터 이름이 CharCreation 화면에 잔류하는 UX 문제 해결
- **[S6' EC-7] item_use 시스템 AccessDenied 패닉**: `#[read_component(Item)]`에서 `#[write_component(Item)]`로 변경. Scroll of enchant weapon/armor 사용 시 `entry_mut().get_component_mut::<Item>()` 호출로 인한 Legion 런타임 AccessDenied 패닉 수정
...
## [2.44.0] - 2026-03-31
### Added
- **[S5c] Throw 선택 UI**: 인벤토리에 Throw 버튼 추가 (Weapon/Potion/Food/Gem). `t` 키 → 인벤토리 열기 → 아이템 선택 → 방향 입력
- **[S5c] ItemAction::Throw**: 인벤토리에서 투척 대상을 선택하는 새 액션 variant

### Changed
- **[S5c] Zap 선택 개선**: Wand 1개면 자동 선택+방향, 2개+이면 인벤토리에서 Zap 버튼으로 선택
- **[S5c] pending_action 분기**: Throw/Zap 선택 시 ActionQueue 대신 `WaitingForDirection` 상태 전환
...
## [2.43.0] - 2026-03-31
### Added
- **[S7-A] ActionQueue pop_typed()**: 타입별 첫 번째 액션만 추출하는 6개 메서드(`pop_item`, `pop_throw`, `pop_cast`, `pop_zap`, `pop_teleport`, `pop_level_change`) 도입. 다중 액션 손실 방지
- **[S7-B] sort_rooms()**: 방 생성 후 좌→우 정렬 + roomno 재할당. 원본 NetHack mklev.c 이식

### Changed
- **[S7-A] 6개 시스템 교체**: item_use, equipment, throw, spell_cast, zap, teleport의 drain-last-one 루프를 pop_typed() 단일 호출로 교체. 코드량 ~70줄 감소
- **[S7-B] 추가 join 축소**: `nroom>4, 1-3개` → `nroom>6, 50% 확률 0-1개`. 맵 과연결 억제
...
## [2.42.5] - 2026-03-31
### Fixed
- **[S6f-1] Swamp 몬스터 캡**: 별도 이중 루프에서 직접 스폰하던 Swamp에도 8마리 상한 적용. Pool 타일 변환은 캡 무관하게 유지

### Note
- 보장 몬스터(Queen Bee, Wraith, Captain 등)는 의도적 설계: helper 캡(8) + 보장(1) = 최대 9마리. 원본 NetHack도 특수방 보스는 별도 배치
...
## [2.42.4] - 2026-03-31
### Fixed
- **[S6e-1] DL3-4 조기 shop 방지**: `makerooms()` 1차 특수방 패스의 shop 조건 `depth>=3` → `depth>=6`. `finalize_special_rooms()`의 `depth>5` 조건과 일치화
- **[S6e-2] 특수방 몬스터 밀도 캡**: `fill_room_with_monsters()`/`fill_room_with_monsters_random()`에 상한 8마리 적용. 기존 Zoo(area=40) 20마리 → 최대 8마리
...
## [2.42.3] - 2026-03-31
### Fixed
- **[S6d-1] 일반방 몬스터 밀도 감소**: `fill_ordinary_room()`의 몬스터 배치를 원본 NetHack 수준으로 조정 — 큰 방 1-3마리/넓은 방 1-2마리 → 방당 33% 확률 0-1마리
- **[S6d-2] MAXNROFROOMS 하드코딩 제거**: `mapseen_ext.rs`의 방 슬롯 40 하드코딩을 `rect::MAXNROFROOMS`(37) 상수 참조로 교체. 코드베이스 일관성 확보
...
## [2.42.2] - 2026-03-31
### Fixed
- **[S6c-1] 장비 액션 큐 소실 해결**: `item_use_system`이 `GameAction::Item`을 전부 꺼낸 뒤 장비류(Wield/Wear/TakeOff 등)이면 return → `equipment_system`에 미도달. 장비 액션은 큐에 보존하도록 필터링 추가
- **[S6c-2] DL2-3 전실 shop 방지**: `finalize_special_rooms()`의 shop 확률 `rn2(depth)<3`이 depth 2-3에서 100% → `depth>5 && rn2(depth)<2`로 수정. 레벨당 특수방 상한 2개 제한 추가
- **[S6c-3] dig_corridor 문 규칙 적용**: 복도 파기 중 벽→문 변환이 `dodoor()`를 우회하던 문제 해결. 인접 문/방별 상한 검사가 복도 생성에도 적용
- **[S6c-4] 점유 검사 MonsterTag 한정**: `makemon()`의 점유 검사가 모든 `Position` 엔티티(아이템/구조물 포함)를 차단 → `MonsterTag` 필터 추가. Minetown watchmen 등 스크립트 배치 정상화
- **[S6c-4] 그룹 스폰 실패 복구**: `m_initgrp()`에서 `makemon()` 실패 시에도 `break 'outer` → 실패 시 다른 인접 칸 시도, 성공 시에만 탈출

### Known Limitations
- 다중 액션 큐 보존: 각 시스템이 큐에서 매칭 타입을 하나만 소비하는 구조. 한 턴 내 복수 동일 타입 액션은 마지막만 실행 (아키텍처 한계)
- split-rect 맵 생성기: 원본 NetHack의 방 생성 알고리즘과 근본 구조가 다름. 수치 조정으로 부분 개선했으나 완전 원본 재현은 별도 리팩토링 필요
...
## [2.42.1] - 2026-03-31
### Fixed
- **[S6b-A] ActionQueue 통합 완료**: `drain_action_queue()`가 시스템 실행 전에 큐를 비워 consumer(throw/equipment/item_use)에 액션이 도달하지 못하던 치명적 버그 해결 — drain 호출 제거
- **[S6b-B] 몬스터 난이도 상한**: `depth+5` → `depth+depth/10+2` (원본 NetHack 기준). DL1에서 레벨6 몬스터가 출현하던 문제 해결
- **[S6b-B] 그룹 스폰 과다**: `m_initgrp()` break가 dy 루프만 끊던 문제 → `break 'outer`로 dx 루프도 탈출. 슬롯당 1마리만 스폰
- **[S6b-B] 몬스터 점유 검사**: `makemon()`에 좌표 점유 검사 추가. 같은 칸에 몬스터 겹쳐 쌓이는 문제 해결
- **[S6b-C] MAXNROFROOMS**: 40→37 (원본 NetHack 기준)
- **[S6b-C] 방 최소 크기**: 폭 clamp(3,...) → clamp(4,...). 너무 좁은 방(3×3) 방지
- **[S6b-C] 문 배치 규칙 강화**: `dodoor()`에 인접 문 검사 + 방별 문 상한(4개) 추가
- **[S6b-C] 이중 인구 방지**: `fill_level_monsters()` 복도 전용 + 수 감소. `finalize_special_rooms()`와의 이중 몬스터 배치 해소
- **[S6b-C] 지형 개성 보존**: Bigroom/Mines/Maze/Minetown에도 공용 랜덤 몬스터 비활성화 (전용 배치 코드 존재)
...
## [2.42.0] - 2026-03-31
### Fixed
- **[Phase S6-1] 입력 경로 통합**: UI/메뉴/마우스/커맨드바에서 설정한 명령이 다음 프레임 poll_input()에 덮어써지는 치명적 버그 해결 — `pending_cmd` 큐 도입으로 모든 입력 소스를 단일 경로로 통합
- **[Phase S6-2] GameState 단일 원천**: `post_turn_processing()`에서 resources의 옛 GameState로 역복원하여 Inventory/Help 등 상태 전환이 즉시 사라지는 결함 수정 — `self.input.game_state`가 유일한 권위원천
- **[Phase S6-3] 턴 비소비 명령 분리**: 인벤토리/도움말 열기만으로 턴이 소비되어 몬스터가 이동하는 문제 해결 — `_action_executed`만 체크하여 시스템 실행
- **[Phase S6-4] 액션 채널 통일 (방향 B)**: Throw/Cast/Zap producer가 `Option<T>` 리소스에 넣고 consumer는 ActionQueue에서 읽는 채널 불일치 수정 — producer가 ActionQueue에 직접 push
- **[Phase S6-5] Render 월드 변경 격리**: Invoke/Fountain/Engrave가 render에서 월드를 직접 변경하고 AI/후처리를 건너뛰는 문제 해결 — `pending_cmd=Wait` 예약으로 턴 소비 파이프라인 합류
- **[Phase S6-6] OfferSelection 이중 렌더링**: 동일 GameState에서 두 개의 Sacrifice 윈도우가 동시에 렌더되는 문제 해결 — 하나로 통합

### Changed
- **리팩토링**: `InputState`에 `pending_cmd`, `pending_action` 필드 추가
- **리팩토링**: `process_game_turn()` 전면 정비 (입력/상태/시스템 실행 흐름 분리)
- **메트릭**: 4,177테스트 전량 통과
...
## [2.40.0] - 2026-02-27
### Added
- **[Phase 103] 5대 모듈 확장**: ascension_승천(8t)/time_시간(9t)/attribute_능력치(8t)/special_특수층(8t)/ghost_고스트(8t) — 총 41테스트, 1,086줄 추가
- **430파일 돌파! 98.8%!**

### Changed
- **메트릭**: 175,136줄 / 430파일 / 4,115테스트 / 이식률 98.8%

## [2.39.0] - 2026-02-27
### Added
- **[Phase 102] 5대 모듈 확장**: inventory_인벤(10t)/ecology_생태(9t)/spellbook_주문(8t)/lighting_조명(9t)/knowledge_식별(9t) — 총 45테스트, 1,315줄 추가
- **98% 돌파! 425파일!**

### Changed
- **메트릭**: 174,050줄 / 425파일 / 4,074테스트 / 이식률 98.2%

## [2.38.0] - 2026-02-27
### Added
- **[Phase 101] 5대 모듈 확장**: quest_퀴스트(8t)/buc_축저(8t)/environ_환경(8t)/strategy_AI전략(8t)/levelup_레벨(8t) — 총 40테스트, 1,055줄 추가
- **🎉 4,000 테스트 돌파! 420파일!**

### Changed
- **메트릭**: 172,735줄 / 420파일 / 4,029테스트 / 이식률 97.5%

## [2.37.0] - 2026-02-27
### Added
- **🏆 [Phase 100] 최종 통합 5대 모듈**: difficulty_난이도(8t)/gameloop_게임루프(8t)/event_이벤트(8t)/mapgen_맵생성(8t)/integration_통합(9t) — 총 41테스트, 1,289줄 추가
- **🎉 Phase 100 도달! 415파일 돌파!**

### Changed
- **메트릭**: 171,680줄 / 415파일 / 3,989테스트 / 이식률 96.9%

## [2.36.0] - 2026-02-27
### Added
- **[Phase 99] 5대 모듈 확장**: save_세이브(7t)/death_사망(7t)/economy_경제(8t)/message_메시지(8t)/artifact_아티팩(9t) — 총 39테스트, 1,229줄 추가
- **파일 410개 돌파 / 96% 돌파!**

### Changed
- **메트릭**: 170,391줄 / 410파일 / 3,948테스트 / 이식률 96.1%

## [2.35.0] - 2026-02-27
### Added
- **[Phase 98] 5대 모듈 확장**: destroy_파괴(8t)/branch_분기(7t)/status_상태(9t)/mkmon_생성(8t)/score_점수(8t) — 총 40테스트, 1,542줄 추가
- **🎉 95% 돌파!**

### Changed
- **메트릭**: 169,162줄 / 405파일 / 3,909테스트 / 이식률 95.4%

## [2.34.0] - 2026-02-27
### Added
- **[Phase 97] 5대 모듈 확장**: eat_음식(8t)/invent_인벤(7t)/read_스크롤(8t)/wand_완드(8t)/options_설정(8t) — 촜 39테스트, 1,377줄 추가
- **파일 400개 돌파!**

### Changed
- **메트릭**: 167,620줄 / 400파일 / 3,869테스트 / 이식률 94.6%

## [2.33.0] - 2026-02-27
### Added
- **[Phase 96] 5대 모듈 확장**: mcastu_마법(8t)/trap_함정(8t)/objnam_식별(8t)/cmd_명령(8t)/vision_시야(9t) — 총 41테스트, 1,429줄 추가

### Changed
- **메트릭**: 166,243줄 / 395파일 / 3,830테스트 / 이식률 93.8%

## [2.32.0] - 2026-02-27
### Added
- **[Phase 95] 5대 모듈 확장**: quest_퀵스트(7t)/mkroom_특수방(7t)/game_bridge_턴(7t)/combat_formula_전투(8t)/sp_lev_특수레벨(8t) — 총 37테스트, 1,531줄 추가

### Changed
- **메트릭**: 164,814줄 / 390파일 / 3,789테스트 / 이식률 93.0%

## [2.31.0] - 2026-02-27
### Added
- **[Phase 94] 5대 모듈 확장**: monmove_AI(9t)/worn_장비(9t)/region_환경(10t)/polymorph_변신(6t)/altar_제단(8t) — 총 42테스트, 1,496줄 추가

### Changed
- **메트릭**: 163,283줄 / 385파일 / 3,752테스트 / 이식률 92.1%

## [2.30.0] - 2026-02-27
### Added
- **[Phase 93] 7대 모듈 대규모 확장**: hack(10t)/spell(9t)/makemon(8t)/apply(8t)/shk(7t)/end(8t)/mklev(7t) — 총 57테스트, 2,826줄 추가

### Changed
- **메트릭**: 161,787줄 / 380파일 / 3,710테스트 / 이식률 91.3%

## [2.29.0] - 2026-02-27 🎉 **90% 돌파 마일스톤!**
### Added
- **[Phase 92] 포션(25종+딥 6결과, 13t) / 던지기(14투사체+부메랑, 7t) / 직업·종족(13직업·5종족+호환성, 9t)**: 총 29테스트, 1,150줄 추가

### Changed
- 🎉 **이식률 90.1% 돌파!** 159,643줄 / 373파일 / 3,653테스트

## [2.28.0] - 2026-02-27
### Added
- **[Phase 91] 10개 모듈 대규모 확장**: mcastu(8t)/trap(11t)/do_name(11t)/teleport(8t)/timeout(10t)/pray(6t)/mhitm(5t)/read(7t)/steal(5t)/detect(7t) — 총 78테스트, 2,648줄 추가

### Changed
- **[메트릭 갱신]** 158,493줄 / 370파일 / 3,624테스트 / 이식률 89.4%

## [2.27.0] - 2026-02-27
### Added
- **[Phase 90-1] 몰스터→플레이어 전투 확장 (`mhitu_phase90_ext.rs`, 신규 385줄, 9테스트)**: 특수공격효과(13종)/패시브반격(5종)/명중판정 — 원본 `mhitu.c` L800-3000.
- **[Phase 90-2] 플레이어→몰스터 전투 확장 (`uhitm_phase90_ext.rs`, 신규 343줄, 9테스트)**: 무기데미지계산(8보너스)/맨손격투(7기술)/치명타판정 — 원본 `uhitm.c` L1300-2800.
- **[Phase 90-3] 음식 시스템 확장 (`eat_phase90_ext.rs`, 신규 302줄, 10테스트)**: 시체효과(14종)/영양가계산/허기상태전이 — 원본 `eat.c` L50-2800.
- **[Phase 90-4] 장비 시스템 확장 (`do_wear_phase90_ext.rs`, 신규 268줄, 11테스트)**: 착용제한(5사유)/장비효과(7종)/해제순서 — 원본 `do_wear.c` L800-2200.
- **[Phase 90-5] 마법 지팡이 확장 (`zap_phase90_ext.rs`, 신규 371줄, 12테스트)**: 빔효과(14빔유형)/지팡이충전소모/빔경로계산 — 원본 `zap.c` L2000-4000.

### Changed
- **[메트릭 갱신]** 155,845줄 / 360파일 / 3,546테스트 / 이식률 87.9%

## [2.26.0] - 2026-02-27
### Added
- **[Phase 1-1] 화면 표시 확장 (`display_phase1_ext.rs`, 신규 403줄, 14테스트)**: 몰스터 표시 판정(투명/변장/석화/경고/환각)/뉴슬심(5레이어 합성)/화면 갱신 범위 결정 — 원본 `display.c` L200-1600 핵심 3개 기능 이식.
- **[Phase 1-2] 커맨드 시스템 확장 (`cmd_phase1_ext.rs`, 신규 393줄, 14테스트)**: 키 바인딩(vi-keys/numpad)/커맨드 분류(반복/이동/턴소모)/이동 유효성 검사/커맨드 도움말 — 원본 `cmd.c` L140-3500 핵심 4개 기능 이식.
- **[Phase 1-3] 옵션 시스템 확장 (`options_phase1_ext.rs`, 신규 336줄, 13테스트)**: 옵션 검증(타입/범위/열거)/제약 테이블/표시 변환/변경 부작용 — 원본 `options.c` L1500-2700 핵심 4개 기능 이식.
- **[Phase 1-4] 세이브/로드 확장 (`save_phase1_ext.rs`, 신규 383줄, 16테스트)**: 스냅샷 검증/메타데이터 생성/자동세이브/차분저장/복구검사 — 원본 `save.c+restore.c` L200-1700 핵심 5개 기능 이식.

### Changed
- **[메트릭 갱신]** 154,180줄 / 355파일 / 3,498테스트 / 이식률 86.9%

## [2.25.0] - 2026-02-26
### Added
- **[Phase 3-1] 시야 시스템 확장 (`vision_phase3_ext.rs`, 신규 695줄, 21테스트)**: Couldsee 테이블/Seenv 방향 비트마스크/Warning 위협 감지 시스템/영역 시야 통계/Shadow Casting FOV(대칭 재귀) — 원본 `vision.c` L100-2541 핵심 5개 기능 이식.
- **[Phase 3-2] 인벤토리 시스템 확장 (`invent_phase3_ext.rs`, 신규 464줄, 23테스트)**: 인벤토리 레터 배정/드롭 검증/짐 용량 계산/아이템 감정(단계별+완전)/아이템 필터링 — 원본 `invent.c` L440-1086 핵심 5개 기능 이식.
- **[Phase 3-3] 상태 바 확장 (`botl_phase3_ext.rs`, 신규 354줄, 11테스트)**: 필드 변경 감지/임계값 색상 시스템/위험 경고 생성 — 원본 `botl.c` L800-1400 핵심 3개 기능 이식.
- **[Phase 3-4] 상점 시스템 확장 (`shk_phase3_ext.rs`, 신규 448줄, 15테스트)**: 상점 주인 행동 결정/가격 흥정/매입 가격 계산/도둑 처벌/상점 서비스 — 원본 `shk.c` L1500-2600 핵심 5개 기능 이식.

### Changed
- **[메트릭 갱신]** 152,469줄 / 351파일 / 3,441테스트 / 이식률 86.0%

## [2.24.0] - 2026-02-26
### Added
- **[Phase 50-1] 채굴 시스템 완성 (`dig_phase50_ext.rs`, 신규 956줄, 31테스트)**: 곡괭이 사용 검증/1턴 채굴 전체판(어질거림/곰함정/노력치)/구멍 생성(액체유입/떨어짐)/위로 파기/상점감시/지팡이 바닥관통 — 원본 `dig.c` L260-1261 핵심 미이식 함수 6개 이식.
- **[Phase 50-2] 몬스터 터널/붕괴/매몰 (`terrain_destroy_ext.rs`, 신규 454줄, 13테스트)**: Umber Hulk/Xorn/Earth Elemental 터널 파기, 터널 붕괴 안정성 판정, 아이템 매몰/발굴 — 원본 `dig.c` L1270-1582 이식.
- **[Phase 50-3] 액체 피해/환경 파괴 (`liquid_damage_ext.rs`, 신규 449줄, 14테스트)**: 용암/물/산성 환경별 아이템 재질(12종) 파괴 판정, 방화/방수/아티팩트 보호, 포션 희석(dilute) — 원본 `dig.c` L1618-1760 이식.

### Changed
- **[메트릭 갱신]** 150,508줄 / 347파일 / 3,376테스트 / 이식률 84.9% / Phase 50 구조물/지형 시스템 완성
...
- **[R34-15] 아이템 생성 확장 (`mkobj_ext.rs`, 신규 ~300줄, 13테스트)**: 클래스 확률 테이블 4종(일반/로그/지옥/상자)/상자 내용물 수/스택 분할/변질 동사 — `mkobj.c` 이식.
- **[R34-16] 행동 확장 (`do_ext.rs`, 신규 ~410줄, 12테스트)**: 드랍 가능 판정(`canletgo`)/제단 BUC 판정/싱크 반지 효과 38종/크리스나이프 퇴화 — `do.c` 이식.
- **[R34-17] 핵심 유틸리티 (`hacklib_ext.rs`, 신규 ~420줄, 25테스트)**: 서수 접미사/부호 함수/반올림 나눗셈/정수 제곱근/일직선 판정/소유격/달의 위상/와일드카드 패턴매칭 3종/퍼지매치/XOR 암호화 — `hacklib.c` 이식.
- **[R34-18] 던전 생성 확장2 (`mklev_ext2.rs`, 신규 ~310줄, 10테스트)**: 광물 배치 확률/특수 방 선택 11종/함정 유형 선택+유효성/가구 배치 확률 — `mklev.c` 이식.
- **[R34-19] 함정 확장2 (`trap_ext2.rs`, 신규 ~320줄, 14테스트)**: 함정 해제 난이도/마법 함정 효과 11종/화재 보호 확률/골렘 화염 취약성/산성 피해 판정 — `trap.c` 이식.
- **[R34-20] 주문 확장2 (`spell_ext2.rs`, 신규 ~370줄, 11테스트)**: 주문 성공률(`percent_success` 완전 이식)/기억 보존율/에너지·허기 비용/역발 효과/주문 유형 분류 — `spell.c` 이식.
- **[R34-21] 상태 표시줄 (`botl_ext.rs`, 신규 ~290줄, 16테스트)**: 힘 수치 문자열 포맷/상태 조건 18종 수집/짐 적재량·단계 5종/성향 문자열/HP 색상 판정 — `botl.c` 이식.
- **[R34-22] 오브젝트 이름 확장2 (`objnam_ext2.rs`, 신규 ~300줄, 14테스트)**: 영어 부정관사(a/an)/복수형 변환 25+ 불규칙 규칙/단수화/글롭 크기 설명/일본어 아이템명 매핑 — `objnam.c` 이식.

### Changed
- **[메트릭 갱신]** 147,120줄 / 338파일 / 3,288테스트 / 이식률 85.2% (+9,050줄, +21파일, +276테스트)
