...
- **결정**: `TurnContext`가 모든 가변 상태를 가지는 안티패턴을 방지하기 위해, 필요에 따라 `CombatContext`, `MovementContext` 등으로 인터페이스를 나누어 권한을 축소한다.
- **배경**: C 원본의 전역 변수 구조(`struct you`, `flags`)를 대체할 목적으로 도입된 `TurnContext`가 비대해질 위험이 큼.
- **근거**: 각 Bridge 모듈이 필요한 최소한의 가변 상태에만 접근하게 하여 시스템 결합도를 낮추고 모듈 독립성을 극대화한다.

### BRIDGE-2. 2-Phase 이벤트 큐 루프 도입
- **결정**: `EventQueue`를 개선하여 이벤트의 즉시 처리(Immediate)와 지연 처리(Deferred)를 구분하는 루프 구조를 도입한다.
- **배경**: 복잡한 이벤트 연쇄(완드 발사 → 폭발 → 상점 적대화 → 플레이어 사망 등)에서 발생할 수 있는 게임 로직과 UI 순서 꼬임 방지.
- **근거**: 상태 변화는 우선 처리하고 UI 렌더링/메시지 출력은 후속으로 일괄 처리하여 정합성을 보장한다.

### BRIDGE-3. Deferred Command (지연 바인딩) 패턴
- **결정**: `TurnContext` 내부의 컴포넌트 동시 수정(Borrow Checker 충돌)을 피하기 위해 `DeferredCommand` 객체를 반환 후 `TurnEngine`의 Apply 단계에서 일괄 처리한다.
- **배경**: 몬스터 AI가 플레이어와 맵 정보를 참조하며 동시에 상태를 수정하려고 할 때 발생하는 Rust의 고질적인 다중 가변 참조 불가 문제를 극복하기 위함.
- **근거**: 상태 변경은 큐에 담아두고 마지막에 적용(Apply)함으로써 엔티티 기반 런타임 레이트 바인딩 안정성을 높임.

### BRIDGE-4. 고급 통합 테스트 전략 (Snapshot, 제한적 FFI, Fuzzing)
- **결정**: 통합 단계의 수치 정합성 보호를 위해 3가지 전략을 혼합 채택한다. 전투/마법 확률 등 난해한 2~3개 모듈에는 **FFI Shadow Testing**을, 엣지 케이스에는 **Snapshot 회귀 테스트**를 적용하고, 순수 함수에는 **cargo-fuzz**를 도입한다.
...
- **결정**: 원본 C 파일이 500줄 이하인 소형 모듈을 xxx_ext.rs라는 독립 확장 모듈로 분리하여 이식한다.
- **배경**: v2.10.1에서 lock.c, steal.c, light.c, bones.c, were.c, rip.c, write.c, minion.c 등 8개 소형 C 파일을 일괄 이식.
- **적용 모듈**: lock_ext.rs, steal_ext.rs, light_ext.rs, bones_ext.rs, were_ext.rs, rip_ext.rs, write_ext.rs, minion_ext.rs
- **결과**: 단일 세션에서 +7,037줄, +181테스트 추가. 이식률 48%52% (50% 돌파).
...
### 결정 #39: ActionQueue 시스템 도입 (Phase R7-1/2/3)
- **결정**: ActionQueue 시스템으로 모든 행동을 계획-실행 분리하고 명령 큐 기반 아키텍처로 전환.
- **근거**: AI와 플레이어의 행동 처리를 분리하고 확장성 있는 명령 채널 확보.
- **결과**: main.rs, pp.rs 등 큐 기반 반영 완료.

### 결정 #40: Pure Result 패턴  CRust 포팅의 새로운 방법론 (Phase R10~R11)

#### 배경
CRust 포팅은 2024~2025년 현재 업계 최대 과제 중 하나이다.
DARPA TRACTOR 프로그램(2024.7), ONCD 메모리 안전 지침(2024.2), CISA 로드맵(2024.10) 등
정부 차원에서까지 CRust 전환을 추진하고 있으나, 핵심 문제가 해결되지 않았다:

- **C2Rust 트랜스파일러**: raw pointer와 unsafe 블록 남발, 비관용적 코드 생산
- **FFI Sandwich**: unsafe 경계가 여전히 존재, UB 전파 위험
- **static mut**: C의 전역 변수가 unsafe 접근 필수인 static mut로 변환

#### Pure Result 패턴 정의
AIHack 프로젝트에서 개발한 CRust 포팅 방법론:

1. **알고리즘 추출**: C 원본에서 순수 알고리즘만 추출 (전역 상태/포인터/부작용 제거)
2. **구조체 입력 캡슐화**: C의 전역 변수  Rust 구조체 매개변수로 변환
3. **Pure 함수 반환**: 부작용 없는 순수 함수로 구현, 결과를 enum/struct로 반환
4. **ECS 분리 적용**: 반환된 Plan/Result를 ECS 레이어에서 적용 (부작용 격리)

#### 기존 방법 대비 차이

| 측면 | C2Rust/FFI | Pure Result |
|------|-----------|-------------|
| unsafe 블록 | 다수 존재 | 0건 |
| 전역 상태 | static mut | struct 입력 |
| 부작용 | 함수 내부에서 직접 수행 | 반환값으로 분리 |
| 테스트 용이성 | Mock 필수, 결합도 높음 | RNG 시드만으로 결정적 검증 |
| borrow checker 충돌 | 빈번 | 원천 차단 |
| 코드 관용성 | 비관용적 | 관용적 Rust |
...
- unsafe 블록: 0건
- unwrap/expect: 0건
- 컴파일 오류: 모든 모듈 1회 수정 내 통과

#### 적용 예시: makemon.c  spawn_ext.rs

C 원본 (makemon.c) - 전역 상태 의존:
- static int mvitals[NUMMONS]; (전역 변수)
- struct monst *makemon() 내부에서 malloc, 전역 맵 수정 등 부작용 직접 수행

Rust Pure Result (spawn_ext.rs) - 순수 함수:
- pub fn plan_makemon(input: &SpawnInput) -> SpawnPlan
- 전역 상태 없음, 구조체 입력, 결과 enum 반환, 부작용 0

#### 논문화 가능성
이 방법론은 다음 관점에서 학술적 가치가 있다:
1. CRust 포팅에서 unsafe 제로를 달성하는 체계적 방법론 제시
...
- **결정**: `execute_turn_systems()` 호출 조건을 `last_cmd != Unknown`에서 `_action_executed`만 체크하도록 변경
- **배경**: 원래는 `last_cmd != Unknown || _action_executed`였으나, UI 전환 커맨드(Inventory, Help 등)도 `last_cmd != Unknown` 조건에 걸려 불필요한 턴 소비가 발생. 인벤토리를 열기만 해도 몬스터가 이동하는 문제.
- **근거**: [v2.42.0 S6-3] UI 전환 커맨드는 `last_cmd = Unknown`으로 소비하여 시스템 미실행. 턴을 소비하는 액션만 `_action_executed = true`를 설정.

---

## [2026-03-31] — Phase S6: 게임 루프 아키텍처 정비 (감사 기반 리팩토링)

### S6-1. 입력 경로 통합 (`pending_cmd`, `pending_action`)

- **결정**: `InputState`에 `pending_cmd: Option<Command>`와 `pending_action: Option<GameAction>` 필드를 추가. UI/메뉴/마우스/커맨드바에서 `self.input.last_cmd`를 직접 건드리지 않고 `pending_cmd`에 예약.
- **배경**: 메인 루프가 `process_game_turn()` → `render_game_ui()` 순서로 실행되므로, render에서 `last_cmd`를 세팅해도 다음 프레임 시작의 `poll_input()`이 덮어씀. UI 클릭, 마우스 이동, 커맨드바 클릭이 모두 무시되는 치명적 버그.
- **근거**: `process_game_turn()` 시작 시 `pending_cmd.take()`가 있으면 우선 사용, 없으면 `poll_input()` 결과 사용. 모든 입력 소스가 하나의 경로로 합쳐짐.

### S6-2. GameState 단일 권위 원천

- **결정**: `self.input.game_state`를 유일한 권위원천으로 지정. `resources.insert(game_state)` → 시스템 실행 직전 한 번만. `post_turn_processing()`의 역복원(`resources.get::<GameState>() → self.input.game_state`) 삭제.
- **배경**: 22행에서 resources에 복사 → 입력 처리 중 game_state 변경 → 1386행에서 resources의 옛 상태로 역복원하여 Inventory/Help 등 상태 전환이 즉시 사라지는 문제.
- **근거**: 시스템이 GameState를 읽기만 하므로, 시스템 실행 직전에만 resources에 복사하면 충분. 쓰기는 self.input.game_state만.

### S6-4. 액션 채널 통일 (방향 B)

- **결정**: Throw/Cast/Zap producer가 `resources.insert(Some(T))` 대신 `action_queue.push(GameAction::T)`로 ActionQueue에 직접 push. 인벤토리/Looting/Offer 등 UI에서는 `pending_action`에 예약.
- **배경**: producer(game_loop)는 `Option<ThrowAction>` 리소스를 사용하고, consumer(throw 시스템)는 `ActionQueue`에서 읽어서 채널 불일치 → 방향 선택 후 효과 미발생.
- **근거**: 시스템 코드(4177 테스트 커버)를 건드리지 않고, producer만 수정하는 방향 B가 리스크 최소.

### S6-5. Render 단계 월드 변경 격리

- **결정**: Invoke/Fountain/Engrave가 render에서 월드를 직접 변경한 후, `pending_cmd = Some(Command::Wait)`를 설정하여 다음 프레임에서 턴 소비 + AI/후처리 실행.
- **배경**: Engrave는 UI에서 턴 수만 수동 증가(`*t += 1`)하고 AI/상태이상/사후처리를 전혀 태우지 않음. Invoke/Fountain도 동일.
- **근거**: 월드 변경 로직 자체를 game_loop으로 이전하면 리팩토링 범위가 과대. `Wait` 커맨드 예약으로 최소 침습적으로 턴 소비 파이프라인에 합류.

---

## [2026-03-31] — Phase S6b: 2차 감사 기반 정비 (ActionQueue + 스폰 + 맵)

### S6b-A. drain_action_queue() 제거

- **결정**: `process_game_turn()` 내 `drain_action_queue()` 호출을 제거. 시스템(throw/equipment/item_use)이 `ActionQueue`에서 직접 `pop()`하므로, 중간에서 큐를 비우는 drain이 불필요하며 오히려 액션 전달을 차단.
- **배경**: S6에서 producer를 ActionQueue.push()로 통일했으나, drain이 시스템 실행 직전에 큐를 비워 `Option<T>` 리소스로 옮기므로 시스템이 도착했을 때 큐가 비어있음.
- **근거**: drain을 제거하면 시스템이 ActionQueue에서 직접 소비하는 정상 경로로 복귀.

### S6b-B. 몬스터 스폰 규칙 원본 정합

- **결정**: 난이도 상한을 `depth + depth/10 + 2`로, 그룹 스폰 break를 `break 'outer`로, `makemon()`에 좌표 점유 검사 추가.
- **배경**: `depth+5` 상한은 DL1에서 레벨6 몬스터를 허용하여 난이도 곡선 파괴. break가 dy만 탈출하여 슬롯당 3마리까지 생성. 점유 검사 없이 같은 칸에 겹침.
- **근거**: 원본 NetHack의 `makemon.c`/`mon.c` 기준에 맞추어 초반 생존률 정상화.

### S6b-C. 맵 생성 규칙 정비

- **결정**: MAXNROFROOMS=37, 방 폭 최소 4, dodoor()에 인접 문/방별 상한, fill_level_monsters()를 복도 전용 + 분기 레벨 제외.
- **배경**: 방이 40개까지 생성되며 3×3 좁은 방이 허용, 문이 한 벽면에 다수 생성, finalize_special_rooms()와 fill_level_monsters() 이중 인구로 "초반부터 수십 마리" 문제.
- **근거**: 원본 `mklev.c`의 방 규칙(37개, 4×3 이상)과 `mkroom.c`의 문 규칙(okdoor)을 반영. 이중 인구는 fill_level_monsters를 복도 보충용으로 축소.

---

## [2026-03-31] — Phase S6c: 3차 감사 기반 정비

### S6c-1. item_use → equipment 큐 보존

- **결정**: `item_use_system`의 ActionQueue drain에서 장비 액션(Wield/Wear/TakeOff/EquipOffhand/QuiverSelect)을 `to_keep`에 보존하여 `equipment_system`이 소비할 수 있게 함.
- **배경**: `item_use`가 모든 `GameAction::Item`을 pop → 장비류이면 `return` → 큐에 돌려놓지 않아 `equipment_system`에 미도달.
- **근거**: 시스템 실행 순서(item_use → equipment)를 변경하지 않고, drain 단계에서 필터링하여 최소 침습적 해결.

### S6c-2. finalize_special_rooms shop 확률 정규화

- **결정**: shop 조건을 `depth>5 && rn2(depth)<2`로 변경하고, 레벨당 특수방 상한 2개 제한 추가.
- **배경**: `rn2(depth)<3`은 depth 2-3에서 100% shop → 초반 모든 방이 상점으로 전환.
- **근거**: 원본 NetHack의 `goldnlev >= 3+rn2(5)` 기준(최소 depth 3~7)을 단순화 반영.

### S6c-3. dig_corridor → dodoor 경유

- **결정**: `dig_corridor()`에서 벽→문 직접 변환을 `dodoor()` 호출로 교체.
- **배경**: dodoor에 인접 문/방별 상한을 추가했으나, 복도 생성기가 dodoor를 우회하여 벽을 직접 Door로 변환.

### S6c-4. 점유 검사 MonsterTag 한정 + 그룹 스폰 복구

- **결정**: `makemon()` 점유 검사를 `(Position, MonsterTag)` 쿼리로 한정. `m_initgrp()`에서 `break 'outer`를 성공 시에만 적용.
- **배경**: 모든 Position 엔티티(아이템, 구조물, 플레이어)를 점유로 판정하여 스크립트 배치(watchmen 등)가 실패. 그룹 스폰은 첫 후보 칸 실패 시 즉시 탈출하여 다른 칸 미시도.

---

## [2026-03-31] — Phase S6d: 4차 감사 기반 정비

### S6d-1. 일반방 몬스터 밀도 원본 수준화

- **결정**: `fill_ordinary_room()`의 몬스터 배치를 방당 33% 확률 0-1마리로 축소.
- **배경**: 기존은 큰 방 1-3마리, 넓은 방 1-2마리로 원본 NetHack보다 2-3배 과밀. 원본에서 일반방은 대부분 비어있고, `fill_level_monsters()`가 레벨 전체에 소수만 배치.

### S6d-2. MAXNROFROOMS 상수 일관성

- **결정**: `mapseen_ext.rs`의 하드코딩 `40`을 `rect::MAXNROFROOMS` 상수 참조로 교체.
- **배경**: rect.rs에서 37로 변경했으나 mapseen은 40 하드코딩 잔류. 소비 측 가정과 생성 측 규칙 불일치.

---

## [2026-03-31] — Phase S6e: 5차 감사 기반 정비

### S6e-1. makerooms() shop 조건 일치화

- **결정**: `makerooms()` 1차 특수방 패스의 shop 조건 `depth >= 3` → `depth >= 6`.
- **배경**: `finalize_special_rooms()`는 `depth > 5`로 수정했으나, `makerooms()`의 1차 패스는 `depth >= 3`으로 남아 DL3-4에서 여전히 shop 생성.

### S6e-2. 특수방 몬스터 밀도 캡

- **결정**: `fill_room_with_monsters()`/`fill_room_with_monsters_random()`의 count에 `.min(8)` 적용.
- **배경**: Zoo(factor 5)/AntHole(factor 6) 등에서 area=40 방이면 20-24마리까지 생성. 원본 NetHack Zoo는 6-12마리 수준.

---

## [2026-03-31] — Phase S6f: 6차 감사 기반 정비

### S6f-1. Swamp 몬스터 캡

- **결정**: Swamp의 별도 이중 루프에 `swamp_spawned` 카운터로 8마리 상한 적용. Pool 타일 변환은 캡 무관.
- **배경**: 공용 helper에만 `.min(8)` 캡을 적용했으나, Swamp는 자체 루프로 몬스터를 직접 배치하여 캡 우회.

### S6f-2. 보장 몬스터 설계 의도

- **결정**: helper 캡(8) + 보장 몬스터(1) = 최대 9마리를 **의도적 설계로 유지**.
- **근거**: 원본 NetHack도 특수방 보스(Wraith, Queen Bee, Captain 등)는 방 인구와 별도로 보장 배치.

### Known Limitations (아키텍처 한계 — ~~수정 보류~~ S7에서 부분 해결)

1. **split-rect 맵 생성기**: 방 배치 알고리즘 자체는 그대로이나, **sort_rooms()**(좌→우 정렬 + roomno 재할당) 및 **추가 join 축소**(nroom>6, 50% 확률 0-1개)로 과연결 억제. 근본 교체는 별도 에픽.
2. **다중 액션 큐 손실**: **pop_typed()** 도입으로 해결. 첫 번째 매칭 액션만 추출, 나머지 보존. 동일 타입 2번째는 다음 턴에 소비.

---

## [2026-03-31] — Phase S7: 아키텍처 리팩토링

### S7-A. ActionQueue pop_typed() 도입

- **결정**: `pop_item()`, `pop_throw()`, `pop_cast()`, `pop_zap()`, `pop_teleport()`, `pop_level_change()` 6개 메서드 추가. 큐에서 해당 타입의 첫 번째만 꺼내고 나머지 보존.
- **배경**: 기존 drain-last-one 패턴은 큐를 전부 비운 후 마지막 매칭 액션만 사용, 나머지 소실.
- **영향**: 6개 시스템(item_use, equipment, throw, spell_cast, zap, teleport) 모두 교체. 코드량 ~70줄 감소.

### S7-B. sort_rooms() + 추가 join 축소

- **결정**: makerooms() 후 방을 좌→우 정렬(`sort_by_key(|r| r.lx)`) + grid roomno 재할당.
- **이유**: 인접 방 연결(`join(i, i+1)`)이 지리적으로 가까운 방끼리 이루어져 복도가 맵 전체를 가로지르는 현상 완화.
- **추가 join**: `nroom>4, 1-3개` → `nroom>6, 50% 확률 0-1개`로 축소.

---

## [2026-03-31] — Phase S5c: 인벤토리/전투/마법 명령어 연결

### S5c-1. Throw 선택 UI

- **결정**: `t` 키 → 인벤토리 열기(Throw 버튼 표시) → 아이템 선택 → 방향 입력.
- **이전**: 첫 번째 아이템 자동 선택 → 방향 입력. 선택 불가능.
- **구현**: `ItemAction::Throw(Entity)` 추가, Weapon/Potion/Food/Gem에 Throw 버튼.

### S5c-2. Zap Wand 선택 개선

- **결정**: Wand 1개→자동 선택, 2개+→인벤토리 열기.
- **이유**: 복수 Wand 보유 시 선택할 수 없는 문제 해결.

### S5c-3. pending_action Throw/Zap 분기

- **결정**: `pending_action`에서 Throw/Zap 감지 시 ActionQueue 대신 `WaitingForDirection` 전환.
- **이유**: 방향 입력이 필요한 액션은 ECS 시스템이 아닌 game_loop에서 상태 머신 전환해야 함.

---

## [2026-04-02] — Phase S6': Edge Case 방어

### EC-1. restart_game() 리소스 완전성 복구

- **결정**: `restart_game()`의 리소스 등록을 `initialize_game_with_choices()`와 완전 일치시킨다.
- **배경**: GameOver → "Restart Now" → CharCreation → Playing 경로에서 첫 턴 시스템 실행 시, `restart_game()`에서 누락된 7개 리소스(AssetManager, Options, VisionSystem, DefaultInteractionProvider, PendingAltarUpdate, TeleportAction, StatusFlags)로 인해 런타임 패닉 발생.
- **근거**: `restart_game()`은 `initialize_game_with_choices()`보다 먼저 작성되어 이후 추가된 리소스가 반영되지 않았음. 같은 목적의 두 함수가 다른 리소스 집합을 등록하는 것은 시간 폭탄.
- **수정**: Grid + VisionSystem + AssetManager + Options + DefaultInteractionProvider + Pray/Teleport 리소스 12개 완비.

### EC-2. SaveManager::load() 리소스 보완

- **결정**: `SaveManager::load()`에서 반환하는 Resources에 런타임 전용 리소스 6개를 추가 등록한다.
- **배경**: Save → Quit → 재시작 → Load 경로에서 ActionQueue, DeathResults, StatusFlags, TeleportAction, PendingAltarUpdate, DefaultInteractionProvider가 누락. 이들은 SaveState에 직렬화되지 않는 런타임 전용 데이터이므로 Load 후 재생성 필요.
- **근거**: `save.rs::load()`는 v2.20.0에서 마지막으로 수정되었고 이후 추가된 시스템(ActionQueue, DeathResults 등)의 리소스가 반영되지 않았음.

### EC-3. restart_game() game_initialized 리셋

- **결정**: `restart_game()` 호출 시 `game_initialized = false`로 리셋한다.
- **배경**: GameOver → "Back to Title" → restart_game() 호출 후 game_initialized가 true로 남아있으면, Title 화면에서 "Continue"를 눌렀을 때 Player 엔티티 없이 Playing 상태에 진입하여 모든 시스템이 패닉.
- **근거**: restart_game()은 World를 새로 생성하므로 Player 엔티티가 없는 상태. Playing 진입은 반드시 CharCreation → initialize_game_with_choices()를 거쳐야 함.

### EC-5. Load 분기 AssetManager/Options Resources 주입

- **결정**: `app.rs` Save Load 분기에서 `resources.insert(assets.clone())` + `resources.insert(options)` 추가.
- **배경**: `SaveManager::load()`는 AssetManager와 Options를 직렬화하지 않으므로 반환된 Resources에 포함되지 않음. 시스템에서 `resources.get::<AssetManager>()`를 호출하면 None을 반환하여 아이템/몬스터 참조 시 패닉.

### EC-6. restart_game() char_name_buf 초기화

- **결정**: `restart_game()` 내에서 `char_name_buf.clear()` 호출 추가.
- **배경**: 이전 게임 캐릭터 이름이 CharCreation 화면에 잔류하여 UX 혼란 유발.

### EC-7. item_use 시스템 #[read_component(Item)] → #[write_component(Item)]

- **결정**: `item_use` 시스템의 컴포넌트 접근 선언을 `#[read_component(Item)]`에서 `#[write_component(Item)]`로 변경.
- **배경**: "Scroll of enchant weapon" 또는 "Scroll of enchant armor" 사용 시, `world.entry_mut(w_ent).get_component_mut::<Item>()`을 호출하여 아이템의 `spe` 값을 수정. 그러나 Legion은 `#[read_component(Item)]`으로 선언된 시스템에서 `entry_mut()`를 통한 write 접근을 거부하여 `AccessDenied` 런타임 패닉을 발생시킴.
- **근거**: `#[read_component]`는 읽기 접근만 허용. `entry_mut().get_component_mut()`는 쓰기 접근이므로 반드시 `#[write_component]`로 선언해야 함. 도구 체인이 컴파일 타임에 잡지 못하는 Legion 특유의 런타임 접근 제어.