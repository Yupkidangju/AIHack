...
| **v2.41.1** | **2026-03-29** | **💯 100%** | **게임 플레이 안정화: ECS AccessDenied 해결, 맵 생성/몬스터 스폰 원본화, Grid 동기화** |
| **v2.42.0~v2.42.5** | **2026-03-31** | **💯 100%** | **S6a~S6f: 게임 루프 안정화 16건 결함 수정** |
| **v2.43.0** | **2026-03-31** | **💯 100%** | **S7: ActionQueue pop_typed() + sort_rooms() + 복도 과연결 억제** |
| **v2.44.0** | **2026-03-31** | **💯 100%** | **S5c: Throw/Zap 선택 UI, 인벤토리 명령어 연결** |

---

## 7. 게임 플레이 정상화 로드맵 (STABILIZATION_ROADMAP 연동)

> 이식률 100%를 달성했으나, 이식된 로직이 실제 게임 루프에 연결되지 않은 부분이 다수 존재.
> 아래 순서로 점진적으로 연결 및 안정화를 진행한다.
> 상세한 태스크별 체크리스트는 `STABILIZATION_ROADMAP.md`를 참조.

### 7.1 현재 진행 상태

| Phase | 상태 | 내용 |
|-------|------|------|
| S0~S4 | ✅ 완료 | 빌드/테스트 통과, 앱 기동, 1000턴 무패닉 |
| **S5a** | **✅ 완료** | **Grid 이중화(Dual Grid) 근본 수정 — 역동기화 구현** |
| **S5b** | **✅ 완료** | **기본 상호작용 9개 동사 시각적 검증 완료** |
| **S5c** | **✅ 완료** | **Throw/Zap 선택 UI, 인벤토리 명령어 연결 (v2.44.0)** |
| **S6** | **✅ 완료** | **게임 루프 안정화 16건 결함 + 아키텍처 리팩토링 (v2.42.0~v2.43.0)** |
| S6' | ⬜ 대기 | Edge Case 방어 (세이브/로드, 레벨 변경 등) |
| S7' | ⬜ 대기 | LLM 최소 주입 |

### 7.2 핵심 기술 과제

1. **Grid 이중화 해결 (v2.41.1 완료)**: `self.game.grid`(렌더러)와 `resources.Grid`(ECS 시스템) 간 역동기화 구현
2. **시작 장비 자동 장착 (v2.41.1 완료)**: 원본 NetHack과 동일하게 초기 무기/방패 자동 장착
3. **GameState 상태 머신 정합성**: 방향 입력 → 타겟 선택 → 실행의 흐름에서 상태 꼬임 방지
4. **인벤토리 선택 UI**: Wield/Wear/Eat/Quaff 커맨드를 위한 아이템 선택 GameState 구현 필요
