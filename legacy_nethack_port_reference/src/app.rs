...
pub(crate) struct InputState {
    ...
    /// [v2.42.0 S6] UI/메뉴/마우스에서 예약된 커맨드 (다음 프레임에 처리)
    /// render_game_ui()에서 last_cmd를 직접 건드리지 않고 여기에 예약
    pub(crate) pending_cmd: Option<crate::ui::input::Command>,
    /// [v2.42.0 S6] UI에서 생성된 게임 액션 (인벤토리 Wield/Eat 등)
    /// render 단계에서 ActionQueue에 직접 넣지 않고 여기에 예약
    pub(crate) pending_action: Option<crate::core::action_queue::GameAction>,
    /// 마법 주문 단축키 입력 (a-z)
...
impl NetHackApp {
    pub fn new(_cc: &eframe::CreationContext<'_>) -> Self {
        ...
        if std::path::Path::new(save_path).exists() {
            ...
            match crate::core::save::SaveManager::load(save_path) {
                Ok((world, mut resources, dungeon)) => {
                    ...
                    // [v2.44.1 S6' EC-5] Load 시 AssetManager/Options 주입
                    // SaveState에 포함되지 않는 런타임 데이터를 Resources에 재주입
        ...
        // [v2.41.1] 시작 아이템 — TOML에 등록된 정확한 이름 사용
        let starting_items = [
            "long sword",      // 무기 (Weapon)
            "orcish shield",   // 방패 (Armor - Shield)
            "leather armor",   // 갑옷 (Armor - Body)
            "Potion of healing",
            "food ration",
            "oil lamp",
        ];
        ...
        for item_name in starting_items {
                ...
                crate::core::entity::spawn::Spawner::mksobj(item_name, &assets.items, &mut world)
            {
                ...
                eprintln!("[디버그] 시작 아이템 '{}' 생성 성공 (Entity: {:?})", item_name, item_ent);
            } else {
                // 아이템 생성 실패 시 경고 로그
                eprintln!("[경고] 시작 아이템 '{}' 생성 실패 — TOML에 이름이 없음", item_name);
            }
        }
        eprintln!("[디버그] 총 시작 아이템: {}개", inventory_items.len());
        ...
        // [v2.41.1] 인벤토리 아이템의 클래스를 확인하여 자동 장착
        {
            ...
            if let Some((equip, inv)) = equip_query.iter_mut(&mut world).next() {
                for (item_ent, class, armor_type) in &item_classes {
                    ...
                    // [v2.41.1] 원본 NetHack에서는 장착 중인 아이템도 인벤토리에 유지
                    // show_inventory()에서 (wielded)/(being worn) 마커로 표시
                    let _ = equipped;
                }
            }
        }
    ...
    /// [v2.44.1 S6'] 재시작 — initialize_game_with_choices()와 동일한 리소스 완비
    /// EC-1: 누락 리소스 12개 추가 (패닉 방지)
    /// EC-3: game_initialized = false (플레이어 없이 Playing 진입 방지)
    /// EC-6: char_name_buf 초기화
    pub(crate) fn restart_game(&mut self) {
        ...
        // [S6'] 리소스 등록 — initialize_game_with_choices()와 완전 일치 보장
        resources.insert(grid.clone());
        resources.insert(crate::core::systems::vision::VisionSystem::new());
        resources.insert(rng.clone());
        resources.insert(assets.clone()); // [EC-1] AssetManager 등록
        resources.insert(self.input.options.clone()); // [EC-1] Options 등록
        ...
        resources.insert(None::<crate::core::systems::pray::PendingAltarUpdate>); // [EC-1] 기도 시스템
        resources.insert(None::<crate::core::systems::teleport::TeleportAction>); // [EC-1] 트랩 텔레포트
        resources.insert(crate::core::entity::status::StatusFlags::empty()); // [EC-1] 상태 플래그
        ...
        resources.insert(crate::core::systems::social::DefaultInteractionProvider); // [EC-1] Talk/Pray
        ...
        // [EC-3] 게임 미초기화 상태로 리셋 — Title→Continue에서 플레이어 없이 진입 방지
        self.ctx.game_initialized = false;
        // [EC-6] 이름 입력 버퍼 초기화
    ...
    pub(crate) fn initialize_game_with_choices(
        ...
        // 2. 직업별 초기 아이템 (role.c 기반)
        let starting_items = match role {
            crate::core::role::Role::Barbarian => vec!["two-handed sword", "leather armor", "food ration"],
            crate::core::role::Role::Knight => {
                vec!["long sword", "spear", "chain mail", "large shield", "food ration"]
            }
            crate::core::role::Role::Wizard => vec!["quarterstaff", "elven cloak", "Scroll of teleportation", "Wand of light", "blank paper"],
            crate::core::role::Role::Rogue => vec!["short sword", "dagger", "dagger", "dagger", "leather armor", "key"],
            crate::core::role::Role::Ranger => vec!["dagger", "elven arrow", "elven arrow", "elven cloak", "food ration"],
            crate::core::role::Role::Valkyrie => vec!["long sword", "dagger", "small shield", "food ration"],
            crate::core::role::Role::Archeologist => vec!["bullwhip", "leather armor", "pick-axe", "tin", "tin", "lamp"],
            crate::core::role::Role::Caveman => vec!["club", "leather armor", "food ration", "boulder"],
            crate::core::role::Role::Healer => vec!["quarterstaff", "leather armor", "Potion of healing", "Potion of healing", "Potion of healing"],
            crate::core::role::Role::Monk => vec!["food ration", "food ration", "food ration", "Potion of healing"],
            crate::core::role::Role::Priest => vec!["mace", "chain mail", "small shield", "water", "water"],
            crate::core::role::Role::Samurai => vec!["long sword", "short sword", "chain mail", "ya", "ya"],
            crate::core::role::Role::Tourist => {
                vec!["dart", "dart", "dart", "food ration", "food ration", "brass lantern"]
            }
        };
        ...
        for item_name in &starting_items {
            if let Some(item_ent) =
                crate::core::entity::spawn::Spawner::mksobj(item_name, &assets.items, &mut world)
            {
                // [v2.44.2] 일부 투사체 및 다량 소요 아이템의 초기 지급량 증대
                if let Ok(mut entry) = world.entry_mut(item_ent) {
                    if let Ok(item) = entry.get_component_mut::<crate::core::entity::Item>() {
                        if item_name.contains("arrow") || item_name.contains("dart") || item_name.contains("ya") || item_name.contains("shuriken") {
                            item.quantity = rng.rnz(20, 40) as u32; // 20~60 개 지급
                        } else if item_name == &"dagger" {
                            item.quantity = rng.rnz(3, 8) as u32; // 단검은 3~11 개 지급
                        } else if item_name == &"tin" || item_name == &"water" {
                            item.quantity = rng.rnz(2, 4) as u32;
                        }
                    }
                }
                inventory_items.push(item_ent);
