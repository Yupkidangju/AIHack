...
struct DungeonDef {
    levels: Vec<(LevelID, Grid)>,
    current_level: LevelID,
    branches: Vec<BranchInfo>,
    difficulty_offset: i32,
    deepest_reached: i32,
    amulet_obtained: bool,
    level_annotations: Vec<(LevelID, String)>,
}

impl serde::Serialize for Dungeon {
    fn serialize<S>(&self, serializer: S) -> Result<S::Ok, S::Error>
    where
        S: serde::Serializer,
    {
        let def = DungeonDef {
            levels: self.levels.clone().into_iter().collect(),
            current_level: self.current_level.clone(),
            branches: self.branches.clone(),
            difficulty_offset: self.difficulty_offset,
            deepest_reached: self.deepest_reached,
            amulet_obtained: self.amulet_obtained,
            level_annotations: self.level_annotations.clone().into_iter().collect(),
        };
        def.serialize(serializer)
    }
}

impl<'de> serde::Deserialize<'de> for Dungeon {
    fn deserialize<D>(deserializer: D) -> Result<Self, D::Error>
    where
        D: serde::Deserializer<'de>,
    {
        let def = DungeonDef::deserialize(deserializer)?;
        Ok(Dungeon {
            levels: def.levels.into_iter().collect(),
            current_level: def.current_level,
            branches: def.branches,
            difficulty_offset: def.difficulty_offset,
            deepest_reached: def.deepest_reached,
            amulet_obtained: def.amulet_obtained,
            level_annotations: def.level_annotations.into_iter().collect(),
        })
    }
}
