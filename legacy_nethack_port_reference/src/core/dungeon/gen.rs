...
impl<'a> LevelGen<'a> {
    ...
    fn makerooms(&mut self) {
        ...
        for _ in 0..MAXNROFROOMS {
            ...
            if let Some(rect) = rect_opt {
                ...
                // [S6b-C] 원본 NetHack 기준: 최소 방 크기 4(폭)×3(높이)
                let r_w = (self.rng.rnz(4, width.min(15) as i32) as usize).clamp(4, width);
        ...
        if nroom > 1 {
            ...
            // [S6e-1] 원본 NetHack 깊이 기반 특수 방 확률
            // shop은 finalize_special_rooms와 일치시켜 depth>=6 (기존 depth>=3)
            ...
            } else if depth >= 6 && self.rng.rn2(4) == 0 {
    ...
    fn makecorridors(&mut self) {
        ...
        //
        // [S7-B] 추가 join 축소: 과연결 억제
        // 기존: nroom>4, 1-3개 → 변경: nroom>6, 50% 확률 0-1개
        if nroom > 6 && self.rng.rn2(2) == 0 {
            let a = self.rng.rn2(nroom as i32) as usize;
            let b = self.rng.rn2(nroom as i32) as usize;
            if a != b {
                self.join(a, b, true);
    ...
    /// [S6b-C] 문 배치 — 인접 문 검사 + 방별 문 상한(4) 추가
    fn dodoor(&mut self, x: usize, y: usize, _room_id: u8) {
        if x >= COLNO || y >= ROWNO || x == 0 || y == 0 {
            return;
        }

        // 인접 칸에 이미 문이 있으면 스킵 (원본 okdoor 대용)
        let adjacents = [(x.wrapping_sub(1), y), (x + 1, y), (x, y.wrapping_sub(1)), (x, y + 1)];
        for (ax, ay) in adjacents {
            if ax < COLNO && ay < ROWNO {
                if self.grid.locations[ax][ay].typ == TileType::Door {
                    // 문 설치가 거부되었으므로 벽을 허물어 길을 개방
                    if self.grid.locations[x][y].typ.is_wall() {
                        self.grid.locations[x][y].typ = TileType::Corr;
                    }
                    return; // 인접에 이미 문 있음
                }
            }
        ...
        // 방별 문 상한 검사 (doorct 사용)
        if _room_id > 0 {
            let room_idx = (_room_id as usize).saturating_sub(1);
            if room_idx < self.rooms.rooms.len() {
                if self.rooms.rooms[room_idx].doorct >= 4 {
                    // 문 설치 제한 걸렸으므로 벽을 복도(개방 통로)로 전환
                    if self.grid.locations[x][y].typ.is_wall() {
                        self.grid.locations[x][y].typ = TileType::Corr;
                    }
                    return; // 방당 최대 4개 문
        ...
        let typ = tile.typ;
        if typ.is_wall() || typ == TileType::Stone {
            ...
            if _room_id > 0 {
                let room_idx = (_room_id as usize).saturating_sub(1);
                if room_idx < self.rooms.rooms.len() {
                    self.rooms.rooms[room_idx].doorct += 1;
                }
            }
        }
    }
    ...
    fn dig_corridor(&mut self, start: usize, end: usize, constant: usize, horizontal: bool) {
        ...
        for pos in min..=max {
            ...
            // [S6c-3] 타일 상태를 먼저 읽고, 조건에 따라 처리
            let typ = self.grid.locations[x][y].typ;
            let roomno = self.grid.locations[x][y].roomno;

            if typ.is_wall() {
                if roomno > 0 {
                    // dodoor()를 통해 인접 문/방별 상한 검사 적용
                    self.dodoor(x, y, roomno);
                } else {
                    self.grid.locations[x][y].typ = TileType::Corr;
                }
            } else if typ == TileType::Stone {
                self.grid.locations[x][y].typ = TileType::Corr;
            }
        }
    }
    ...
    fn fill_level_monsters(
        ...
        // [S6b-C] 전용 배치 코드가 있는 레벨 타입은 공용 랜덤 몬스터 추가 제외
        if matches!(
            level_type,
            LevelType::Sokoban
                | LevelType::Bigroom
        ...
        // [S6b-C] finalize_special_rooms()가 이미 방 안에 몬스터를 채움
        // fill_level_monsters()는 복도 + 소수 보충만 (이중 인구 방지)
        let target_monsters = ((depth / 6) as usize + 1).min(nroom);
        ...
        while spawned < target_monsters && attempts < target_monsters * 10 {
            ...
            // [S6b-C] 복도에만 스폰 (방 안은 finalize_special_rooms가 이미 처리)
            if tile_typ == TileType::Corr {
    ...
    pub fn generate_improved(
        ...
        match level_type {
            LevelType::Ordinary => {
                ...
                // [S7-B] sort_rooms: 좌→우 정렬 후 roomno 재할당
                // 원본 NetHack mklev.c의 sort_rooms() 이식
                gen.rooms.rooms.sort_by_key(|r| r.lx);
                // 정렬 후 grid의 roomno 재할당
                for (idx, room) in gen.rooms.rooms.iter().enumerate() {
                    let new_id = (idx as u8) + 1;
                    for x in room.lx..=room.hx {
                        for y in room.ly..=room.hy {
                            if x < crate::core::dungeon::COLNO && y < crate::core::dungeon::ROWNO {
                                gen.grid.locations[x][y].roomno = new_id;
                            }
                        }
                    }
                }
