...
impl RoomManager {
    ...
    pub fn finalize_special_rooms(
        ...
        // [S6c-2] 레벨당 특수방 상한 (makerooms에서 이미 1개 설정 가능 → 여기서 최대 1개 추가)
        let mut special_count = self.rooms.iter().filter(|r| r.rtype != RoomType::Ordinary).count();
        ...
        for i in 0..nroom {
            ...
            // [S6c-2] 특수방 상한: 전체 2개까지만 (makerooms 1개 + 여기 1개)
            let rtype = if special_count >= 2 {
                None
            } else if depth > 5 && depth < 20 && nroom >= room_threshold && rng.rn2(depth) < 2
            {
                // [S6c-2] 원본 NetHack 기준: shop은 depth>5, 확률 rn2(depth)<2
            ...
            if let Some(rt) = rtype {
                ...
                special_count += 1; // [S6c-2] 특수방 카운터 증가
            } else {
    ...
    pub fn fill_ordinary_room(
        ...
        // [S6d-1] 원본 NetHack 기준: 일반방은 대부분 비어있음
        // 방당 ~33% 확률로 몬스터 1마리만 (기존: 큰 방 1-3, 넓은 방 1-2)
        let monster_count = if rng.rn2(3) == 0 { 1 } else { 0 };
    ...
    pub fn mkspecial(
        ...
        match rtype {
            ...
            RoomType::Swamp => {
                // [S6f-1] Swamp 몬스터 캡 적용 (Pool 변환은 제한 없이 유지)
                let mut swamp_spawned = 0usize;
                for x in room.lx + 1..room.hx {
                    for y in room.ly + 1..room.hy {
                        if (x + y) % 2 == 0 && rng.rn2(2) == 0 {
                            ...
                            if swamp_spawned < 8 && rng.rn2(3) == 0 {
                                Spawner::makemon_of_class(
                                    if rng.rn2(2) == 0 { ';' } else { 'F' },
                                    x,
                                    y,
                                    NO_MM_FLAGS,
                                    world,
    ...
    fn fill_room_with_monsters(
        ...
        // [S6e-2] 특수방 몬스터 밀도 캡 적용
        // 원본 NetHack: Zoo/Beehive 등은 6-12마리 수준
        let area = room.area();
        let count = ((area / 10).max(1) * density_factor).min(8);
