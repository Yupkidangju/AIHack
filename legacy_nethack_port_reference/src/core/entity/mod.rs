...
impl serde::Serialize for Inventory {
    ...
    where
        S: serde::Serializer,
    {
        let def = InventoryDef {
            items: self.items.clone(),
            letter_map: self.letter_map.clone().into_iter().collect(),
        };
...
impl<'de> serde::Deserialize<'de> for Inventory {
    ...
    {
        let def = InventoryDef::deserialize(deserializer)?;
        Ok(Inventory {
            items: def.items,
            letter_map: def.letter_map.into_iter().collect(),
        })
    }
}
...
#[derive(Serialize, Deserialize)]
struct EquipmentDef {
    slots: Vec<(EquipmentSlot, Entity)>,
}
...
impl<'de> serde::Deserialize<'de> for Equipment {
    ...
    where
        D: serde::Deserializer<'de>,
    {
        let def = EquipmentDef::deserialize(deserializer)?;
        Ok(Equipment {
            slots: def.slots.into_iter().collect(),
        })
    }
}
...
#[derive(Serialize, Deserialize)]
struct SpellKnowledgeDef {
    spells: Vec<(char, LearnedSpell)>,
}

impl serde::Serialize for SpellKnowledge {
    fn serialize<S>(&self, serializer: S) -> Result<S::Ok, S::Error>
    where
        S: serde::Serializer,
    {
        let def = SpellKnowledgeDef {
            spells: self.spells.clone().into_iter().collect(),
        };
        def.serialize(serializer)
    }
}

impl<'de> serde::Deserialize<'de> for SpellKnowledge {
    fn deserialize<D>(deserializer: D) -> Result<Self, D::Error>
    where
        D: serde::Deserializer<'de>,
    {
        let def = SpellKnowledgeDef::deserialize(deserializer)?;
        Ok(SpellKnowledge {
            spells: def.spells.into_iter().collect(),
        })
    }
}
