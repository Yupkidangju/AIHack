...
impl Spawner {
    ...
    pub fn makemon(
        ...
        // [S6b-B] 원본 NetHack 난이도 상한: depth + depth/10 + 2
        let difficulty_cap = level_id.depth + level_id.depth / 10 + 2;
        if (template.level as i32) > difficulty_cap && (mmflags & MM_IGNLEVELCHECK == 0) {
            return None;
        }
        // [S6c-4] 점유 검사: 해당 칸에 이미 *몬스터*가 있으면 스킵 (goodpos 대용)
        // 아이템/구조물/플레이어는 점유로 취급하지 않음
        if x < crate::core::dungeon::COLNO && y < crate::core::dungeon::ROWNO {
            let mut occupied = false;
            let mut pos_query = <(&crate::core::entity::Position, &MonsterTag)>::query();
            for (pos, _) in pos_query.iter(world) {
                if pos.x == x as i32 && pos.y == y as i32 {
                    occupied = true;
                    break;
                }
            }
            if occupied && (mmflags & MM_IGNLEVELCHECK == 0) {
                return None;
            }
        }
    ...
    fn select_monster_by_difficulty<'a>(
        ...
        for t in templates {
            ...
            // [S6b-B] 원본 NetHack 난이도 상한: depth + depth/10 + 2
            let difficulty_cap = depth + depth / 10 + 2;
            if (t.level as i32) > difficulty_cap {
                continue;
            }
                ...
                for dy in -1..=1 {
                    ...
                    if nx < COLNO && ny < ROWNO {
                        if let Some(tile) = grid.get_tile(nx, ny) {
                            if tile.typ == TileType::Room {
                                // [S6c-4] makemon 실패 시 다른 칸 시도 (continue)
                                // 성공 시에만 break 'outer
                                if let Some(child_ent) = Self::makemon(
                                    ...
                                    break 'outer; // 성공 시만 탈출
                                }
                                // 점유 등으로 실패 → 다른 인접 칸 시도
                            }
                        }
                    }
                }
