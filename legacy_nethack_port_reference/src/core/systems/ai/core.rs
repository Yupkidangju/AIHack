...
pub fn monster_ai(
    ...
    // [v2.0.0
    let mut occupancy: HashSet<(i32, i32)> = <(&Position, &MonsterTag, &Level)>::query()
        .iter(world)
        .filter(|(_, _, lvl)| lvl.0 == p_level.0)
        .map(|(pos, _, _)| (pos.x, pos.y))
        .collect();
    // 플레이어의 위치도 점유(occupancy) 공간으로 추가 (겹침 방지)
    occupancy.insert((p_pos.x, p_pos.y));

    let mut obstacles = occupancy.clone();
    ...
    for (m_entity, m_pos, m_state, monster, m_faction, m_level) in query.iter_mut(world) {
        ...
        if m_state.mflee {
            if m_state.mfleetim > 0 {
                ...
                if (m_pos.x, m_pos.y) != old_pos {
                    occupancy.remove(&old_pos);
                    obstacles.remove(&old_pos);
                    occupancy.insert((m_pos.x, m_pos.y));
                    obstacles.insert((m_pos.x, m_pos.y));
                }
                continue;
            } else {
                m_state.mflee = false;
...
fn move_towards(
    ...
    if let Some(template) = assets.monsters.get_by_kind(monster.kind) {
        ...
        if let Some(path) = AiHelper::get_path(
            ...
            if path.len() > 1 {
                let next = path[1];
                let is_occupied = occupancy.contains(&(next.0, next.1));
                if (!is_pet || !trap_positions.contains(&(next.0, next.1))) && !is_occupied {
