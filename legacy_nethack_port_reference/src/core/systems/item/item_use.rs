...
/// [v2.44.1 S6'] Item 컴포넌트 write 접근 필요 — enchant weapon/armor 스크롤 사용 시
/// #[read_component(Item)]에서 entry_mut() 호출 시 AccessDenied 패닉 발생
#[legion::system]
...
pub fn item_use(
    ...
    // [S7-A] pop_item()으로 비장비 첫 번째 Item 액션 추출
    // 장비 액션이 먼저 나오면 큐에 재삽입하고 다음 Item을 시도
    let mut action_to_process = None;
    let mut equipment_actions = Vec::new();
    while let Some(a) = action_queue.pop_item() {
        match &a {
            ItemAction::Wield(_)
            | ItemAction::Wear(_)
            ...
            | ItemAction::QuiverSelect(_) => {
                // 장비 액션 → 나중에 큐에 되돌림
                equipment_actions.push(a);
            }
            _ => {
                action_to_process = Some(a);
                break; // 비장비 첫 번째 발견 → 중단
            }
        }
    }
    // 건너뛴 장비 액션들을 큐에 재삽입
    for ea in equipment_actions {
        action_queue.push(crate::core::action_queue::GameAction::Item(ea));
    }
    ...
    let item_ent = match action {
        ...
        // [S6c-1] 장비 액션은 큐 필터링 단계에서 이미 보존됨 → 여기 도달 불가
        _ => return,
    };

    // [S6c-1] TakeOut/PutIn/Apply/Name/Call 등 특수 처리
    match action {
