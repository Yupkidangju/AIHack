...
impl super::NetHackApp {
    ...
    pub(crate) fn render_game_ui(&mut self, ctx: &egui::Context) {
        ...
        // (B) 메뉴 바 렌더링 (최상단)
        {
            ...
            match menu_action {
                MenuAction::Inventory => {
                    self.input.pending_cmd = Some(Command::Inventory);
                }
                ...
                MenuAction::Save => {
                    self.input.pending_cmd = Some(Command::Save);
                }
                MenuAction::Quit => {
        ...
        // (C) 커맨드 바 렌더링 (최하단)
        {
            ...
            // [S6-1] 커맨드 바 클릭 → pending_cmd에 예약
            match cmd_action {
                CommandBarAction::Pickup => {
                    self.input.pending_cmd = Some(Command::Pickup);
                }
                CommandBarAction::Inventory => {
                    self.input.pending_cmd = Some(Command::Inventory);
                }
                CommandBarAction::Eat => {
                    self.input.pending_cmd = Some(Command::Eat);
                }
                CommandBarAction::Quaff => {
                    self.input.pending_cmd = Some(Command::Quaff);
                }
                CommandBarAction::Read => {
                    self.input.pending_cmd = Some(Command::Read);
                }
                CommandBarAction::Apply => {
                    self.input.pending_cmd = Some(Command::Apply);
                }
                CommandBarAction::Zap => {
                    self.input.pending_cmd = Some(Command::Zap);
                }
                CommandBarAction::Cast => {
                    self.input.pending_cmd = Some(Command::Cast);
                }
                CommandBarAction::Pray => {
                    self.input.pending_cmd = Some(Command::Pray);
                }
                CommandBarAction::Search => {
                    self.input.pending_cmd = Some(Command::Search);
                }
                CommandBarAction::Help => {
                    self.input.pending_cmd = Some(Command::Help);
                }
                CommandBarAction::Throw => {
                    self.input.pending_cmd = Some(Command::Throw);
                }
                CommandBarAction::Kick => {
                    self.input.pending_cmd = Some(Command::Kick);
                }
                CommandBarAction::Open => {
                    self.input.pending_cmd = Some(Command::Open);
                }
                CommandBarAction::Close => {
                    self.input.pending_cmd = Some(Command::Close);
                }
                CommandBarAction::Wear => {
                    self.input.pending_cmd = Some(Command::Wear);
                }
                CommandBarAction::TakeOff => {
                    self.input.pending_cmd = Some(Command::TakeOff);
                }
                CommandBarAction::Wield => {
                    self.input.pending_cmd = Some(Command::Wield);
                }
                CommandBarAction::Engrave => {
                    self.input.pending_cmd = Some(Command::Engrave);
                }
                CommandBarAction::Name => {
                    self.input.pending_cmd = Some(Command::Name);
                }
                        ...
                        if let Some((gx, gy)) = mouse::screen_to_grid(
                            ...
                            match action {
                                mouse::MouseAction::AdjacentMove { dir, .. } => {
                                    // 인접 이동/공격 (1턴)
                                    self.input.pending_cmd = Some(mouse::direction_to_command(dir));
                                    // Travel 모드 해제
        ...
        if self.input.ext_cmd_mode {
            ...
            if close_popup {
                ...
                if resolved_cmd != Command::Unknown {
                    self.input.pending_cmd = Some(resolved_cmd);
                }
        ...
        if self.input.last_cmd == Command::ExtendedCommand && !self.input.ext_cmd_mode {
            ...
            self.input.ext_cmd_input.clear();
            self.input.pending_cmd = Some(Command::Unknown); // 이번 프레임은 명령 없음
        }
        ...
        if self.input.last_cmd == Command::Unknown {
            if let Some(dir) = self.input.run_direction {
                ...
                if can_continue {
                    self.input.pending_cmd = Some(crate::ui::mouse::direction_to_command(dir));
                } else {
        ...
        // ================================================================
        {
            ...
            if let Some(text) = msg {
                ...
                match self.input.last_cmd {
                    ...
                    | Command::Overview
                    | Command::Monster => {
                        self.input.pending_cmd = Some(Command::Unknown);
                    }
                    _ => {}
                }
            }
            ...
            egui::Window::new("Inventory (i)").show(ctx, |ui| {
                ...
                for (inv, equip) in query.iter(&self.game.world) {
                    ...
                    if let Some(action) = action {
                        // [S6-4] 모든 아이템 액션을 pending_action으로 예약
                        // render 단계에서 ActionQueue에 직접 넣지 않음
                        self.input.pending_action = Some(
                            crate::core::action_queue::GameAction::Item(action),
                        );
                        // 인벤토리 닫기
        ...
        // [S6-6] OfferSelection은 1724행의 "Sacrifice" 창으로 통합됨 (이중 렌더링 제거)
        ...
        if self.input.game_state == GameState::SelectInvoke {
            egui::Window::new("Invoke Artifact").show(ctx, |ui| {
                ...
                if let Some(target) = selected {
                    ...
                    // [S6-5] Invoke는 render에서 월드를 변경하므로, 턴 소비를 위해 Wait 예약
                    self.input.pending_cmd = Some(Command::Wait);
                    self.input.game_state = GameState::Normal;
                }
            ...
            egui::Window::new("Engrave Text").show(ctx, |ui| {
                    ...
                    || ui.button("Write").clicked()
                {
                    ...
                    if let Some(mut log) = log_opt {
                        ...
                        self.input.game_state = GameState::Normal;
                        // [S6-5] 새기기 실행 후 턴 소비 예약 — 수동 턴 증가 대신 게임 루프 경유
                        self.input.pending_cmd = Some(Command::Wait);
                    }
                } // if let Some(mut log)
                if ui.button("Cancel").clicked() {
