...
impl super::NetHackApp {
    ...
    pub(crate) fn handle_title_screen(&mut self, ctx: &egui::Context) {
        ...
        match render_title_screen(ctx) {
            TitleAction::NewGame => {
                //
                self.app_state = crate::core::role::AppState::CharacterCreation {
                    step: crate::core::role::CharCreationStep::SelectRole,
                    choices: crate::core::role::CharCreationChoices::new(),
                };
                self.char_creation_step = crate::core::role::CharCreationStep::SelectRole;
                self.char_creation_choices = crate::core::role::CharCreationChoices::new();
                self.char_name_buf.clear();
            }
            TitleAction::Continue => {
                //
                if self.game_initialized {
                    self.app_state = crate::core::role::AppState::Playing;
                }
            }
            TitleAction::Quit => {
    ...
    pub(crate) fn handle_char_creation_screen(&mut self, ctx: &egui::Context) {
        ...
        match render_char_creation(
            ctx,
            &mut self.char_creation_step,
            &mut self.char_creation_choices,
            &mut self.char_name_buf,
        ) {
            ...
            CharCreationAction::BackToTitle => {
                self.app_state = crate::core::role::AppState::Title;
            }
    ...
    pub(crate) fn check_game_over(&mut self) -> bool {
        if let crate::core::game_state::GameState::GameOver { ref message } = self.game_state {
            //
            ...
            let (score, turns, max_depth, epitaph) = {
                ...
                //
                if let Some(turn) = self.resources.get::<u64>() {
                    turn_count = *turn;
                }
            ...
            self.app_state = crate::core::role::AppState::GameOver {
                message: msg,
                score,
                turns,
                max_depth,
