...
/// [v3.0.0] GameContext 기반 전환 완료 (몬스터 AI 시스템)
pub fn monster_ai(ctx: &mut crate::core::context::GameContext) {
    ...
    let rng = &mut rng;
    // [v3.0.0] vision과 p_status_resists는 ctx에 없으므로 로컬 생성
    let mut vision = VisionSystem::new();
    let p_status_resists = StatusFlags::empty();
    let p_status_resists = &p_status_resists;
    {
        //
        ...
        for (ent, fac, m, pos) in
            <(Entity, &MonsterFaction, &Monster, &Position)>::query().iter(world)
        {
        ...
        for (m_ent, m_name, attack, _, target_ac) in attacks_to_apply {
            ...
            let dmg =
                CombatEngine::calculate_monster_damage(rng, &attack, target_ac, &p_status_flags);
            ...
            if final_dmg > 0 {
                if let Some(mut p_entry) = world.entry(p_entity) {
                    ...
                    if !resisted {
                        match attack.adtype {
                            ...
                            crate::core::entity::monster::DamageType::Were => {
                                let effect =
                                    crate::core::systems::combat::mhitu::lycanthropy_attack(
                                        &m_name.to_string(),
                                        "werewolf",
                                        false,
                                        rng,
                                ...
                                if effect.is_some() {
                                    if let Ok(p_status) =
                                        p_entry.get_component_mut::<StatusBundle>()
                                    {
        ...
        for (m_ent, item_ent) in pickups_to_apply {
            if let Some(mut entry) = world.entry(m_ent) {
                if let Ok(inv) = entry.get_component_mut::<crate::core::entity::Inventory>() {
                    ...
                    if let Some(mut ie) = world.entry(item_ent) {
                        ie.remove_component::<Position>();
                    }
                }
            }
        }
