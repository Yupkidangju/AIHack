...
impl super::NetHackApp {
    ...
    pub(crate) fn process_game_turn(&mut self, ctx: &eframe::egui::Context) {
        ...
        self.last_cmd = polled_cmd;
        self.spell_key_input = polled_spell;
        //

        //
        self.resources.insert(self.game_state.clone());
        ...
        match &self.game_state {
            GameState::More => {
                ...
                if self.last_cmd != Command::Unknown {
                    ...
                    self.last_cmd = Command::Unknown; // 입력 소비
                }
            }
            GameState::WaitingForDirection { action } => {
                // 방향 입력 대기 중
                if self.last_cmd == Command::Cancel {
                    // ESC: 취소
                    self.game_state.reset();
                    if let Some(mut log) = self.resources.get_mut::<crate::ui::log::GameLog>() {
                ...
                } else if let Some(dir) = Direction::from_command(&self.last_cmd) {
                    // 방향 입력 받음 - 액션 실행
                    ...
                    match action_copy {
                        DirectionAction::Throw { item } => {
                            self.resources
                                .insert(Some(crate::core::systems::throw::ThrowAction {
                                    item,
                                    dir,
                                }));
                            self.last_cmd = Command::Unknown; // 입력 소비 (이동 방지)
                        ...
                        DirectionAction::Apply { item } => {
                            let turn = self.resources.get::<u64>().map(|t| *t).unwrap_or(0);
                            let mut cb = legion::systems::CommandBuffer::new(&self.world);

                            {
                                if let Some(mut log) =
                                    self.resources.get_mut::<crate::ui::log::GameLog>()
                                {
                            ...
                            cb.flush(&mut self.world, &mut self.resources);
                            self.resources.insert(self.grid.clone());
                            self.last_cmd = Command::Unknown;
                            _action_executed = true;
                        }
                        ...
                        DirectionAction::Talk => {
                            ...
                            if let Some(mut log) =
                                self.resources.remove::<crate::ui::log::GameLog>()
                            {
                                if let Some(mut rng) =
                                    self.resources.remove::<crate::util::rng::NetHackRng>()
                                {
                                    if let Some(rumors) =
                                        self.resources.get::<crate::core::systems::talk::Rumors>()
                                    {
            ...
            GameState::WaitingForSpell => {
                // 마법 주문 단축키 입력 대기 (a-z)
                if let Some(c) = self.spell_key_input {
                    ...
                    for knowledge in query.iter(&self.world) {
            ...
            GameState::IdentifySelect {
                ...
                if let Some(c) = self.spell_key_input {
                    ...
                    if let Some(inv) = inv_query.iter_mut(&mut self.world).next() {
                        ...
                        if let Ok(mut entry) = self.world.entry_mut(target) {
                            if let Ok(item) = entry.get_component_mut::<crate::core::entity::Item>()
                            {
                                if !item.known || !item.bknown {
                                    ...
                                    {
                                        let turn =
                                            self.resources.get::<u64>().map(|t| *t).unwrap_or(0);
                    ...
                    if count == 0 {
                        ...
                        if let Some(p_inv) = p_inv_query.iter_mut(&mut self.world).next() {
                            if let Some(pos) = p_inv.items.iter().position(|&e| e == scroll) {
                                p_inv.items.remove(pos);
                            }
                            p_inv.letter_map.retain(|_, &mut v| v != scroll);
                        }
                        self.world.remove(scroll);
                        if let Some(mut log) = self.resources.get_mut::<crate::ui::log::GameLog>() {
                            let turn = self.resources.get::<u64>().map(|t| *t).unwrap_or(0);
                            log.add("The scroll crumbles to dust.", turn);
                        }
                    } else {
                        self.game_state = GameState::IdentifySelect { scroll, count };
                    }
                } else if self.last_cmd == Command::Cancel {
                    self.game_state.reset();
                    if let Some(mut log) = self.resources.get_mut::<crate::ui::log::GameLog>() {
                        let turn = self.resources.get::<u64>().map(|t| *t).unwrap_or(0);
                        log.add("Never mind.", turn);
            ...
            GameState::ConfirmRefill { lamp, oil } => {
                ...
                if confirmed {
                    ...
                    if let Some(inv) = inv_q.iter_mut(&mut self.world).next() {
                    ...
                    if oil_used {
                        if let Ok(mut entry) = self.world.entry_mut(*lamp) {
            ...
            GameState::Normal => {
                ...
                match self.last_cmd {
                    ...
                    Command::Open => {
                        self.game_state.request_direction(DirectionAction::Open);
                        if let Some(mut log) = self.resources.get_mut::<crate::ui::log::GameLog>() {
                            let turn = self.resources.get::<u64>().map(|t| *t).unwrap_or(0);
                            log.add("In what direction?", turn);
                        }
                    ...
                    Command::Close => {
                        self.game_state.request_direction(DirectionAction::Close);
                        if let Some(mut log) = self.resources.get_mut::<crate::ui::log::GameLog>() {
                            let turn = self.resources.get::<u64>().map(|t| *t).unwrap_or(0);
                            log.add("In what direction?", turn);
                        }
                    ...
                    Command::Kick => {
                        self.game_state.request_direction(DirectionAction::Kick);
                        if let Some(mut log) = self.resources.get_mut::<crate::ui::log::GameLog>() {
                            let turn = self.resources.get::<u64>().map(|t| *t).unwrap_or(0);
                            log.add("In what direction?", turn);
                        }
                    ...
                    Command::Talk => {
                        self.game_state.request_direction(DirectionAction::Talk);
                        if let Some(mut log) = self.resources.get_mut::<crate::ui::log::GameLog>() {
                            let turn = self.resources.get::<u64>().map(|t| *t).unwrap_or(0);
                            log.add("In what direction?", turn);
                        }
                    ...
                    Command::Cast => {
                        self.game_state = GameState::WaitingForSpell;
                        if let Some(mut log) = self.resources.get_mut::<crate::ui::log::GameLog>() {
                            let turn = self.resources.get::<u64>().map(|t| *t).unwrap_or(0);
                    ...
                    Command::Loot => {
                        ...
                        self.game_state.request_direction(DirectionAction::Loot);
                    }
                    Command::Search => {
                        let (mut subworld, _) =
                            self.world
                                .split_for_query(
                                    &<(&Position, &mut crate::core::entity::Trap)>::query(),
                                );
                        if let Some(mut log) = self.resources.get_mut::<crate::ui::log::GameLog>() {
                            let turn = self.resources.get::<u64>().map(|t| *t).unwrap_or(0);
                            if let Some(mut rng) =
                    ...
                    Command::Offer => {
                        ...
                        if let Some((px, py)) = p_pos {
                            if let Some(tile) = self.grid.get_tile(px as usize, py as usize) {
                                if tile.typ == crate::core::dungeon::tile::TileType::Altar {
                                    self.game_state = GameState::OfferSelection;
                                    if let Some(mut log) =
                                        self.resources.get_mut::<crate::ui::log::GameLog>()
                                    {
                                        let turn =
                                ...
                                if let Ok(entry) = self.world.entry_ref(item_ent) {
                                        ...
                                        entry.get_component::<crate::core::entity::Item>()
                                    {
                                        if let Some(template) =
                                            self.assets.items.get_by_kind(item.kind)
                                        {
                                            if template.class
                    ...
                    Command::Enhance => {
                        self.game_state = GameState::Enhance;
                    }
                    Command::TwoWeapon => {
                        ...
                        if current_state {
                            ...
                            self.game_state = GameState::SelectOffhand;
                        }
                    }
                    Command::Swap => {
                        let mut query = <&mut crate::core::entity::Equipment>::query()
                            .filter(component::<PlayerTag>());
                        for eq in query.iter_mut(&mut self.world) {
                    ...
                    Command::Engrave => {
                        self.game_state = GameState::SelectEngraveTool;
                    }
                    Command::Name => {
                    ...
                    Command::Invoke => {
                        self.game_state = GameState::SelectInvoke;
                        if let Some(mut log) = self.resources.get_mut::<crate::ui::log::GameLog>() {
                            let turn = self.resources.get::<u64>().map(|t| *t).unwrap_or(0);
                    ...
                    Command::Fire => {
                        let mut query = <&crate::core::entity::Equipment>::query()
                            .filter(component::<PlayerTag>());
                        let mut item_to_fire = None;
                        for eq in query.iter(&self.world) {
                            item_to_fire = eq
                                .slots
                    ...
                    Command::Quiver => {
                        self.game_state = GameState::SelectQuiver;
                    }
                    Command::LogHistory => {
            ...
            | GameState::EngravingText { .. } => {
                if self.last_cmd == Command::Cancel || self.last_cmd == Command::Inventory {
                    self.game_state.reset();
                    self.last_cmd = Command::Unknown;
                }
            }
            GameState::Help => {
                if self.last_cmd == Command::Cancel || self.last_cmd == Command::Help {
                    self.game_state.reset();
                }
            }
            GameState::Looting { .. } => {
            ...
            GameState::Enhance => {
                if self.last_cmd == Command::Cancel || self.last_cmd == Command::Enhance {
                    self.game_state.reset();
                }
            }
            _ => (/* Normal 등 */),
        ...
        // 명령이 있을 경우 시스템 실행
        if self.last_cmd != crate::ui::input::Command::Unknown {
            //
            self.resources.insert(self.last_cmd);
            self.resources.insert(self.grid.clone());

            //
            if _action_executed {
                if let Some(mut turn) = self.resources.get_mut::<u64>() {
                    *turn += 1;
                    if let Some(mut log) = self.resources.get_mut::<crate::ui::log::GameLog>() {
            ...
            //
            // [v1.9.0
            if let Some(mut rng) = self.resources.get_mut::<crate::util::rng::NetHackRng>() {
                ...
                let current_turn = self.resources.get::<u64>().map(|t| *t).unwrap_or(0);
                ...
                .get::<crate::core::systems::death::DeathResults>()
            {
                ...
                let current_level = {
                    ...
                    lvl_query.iter(&self.world).next().map(|l| l.0).unwrap_or(
                        crate::core::dungeon::LevelID::new(
            ...
            //
            {
                ...
                if let Some(eq) = self.resources.get::<crate::core::events::EventQueue>() {
                    if !eq.is_empty() {
                        //
                        let events: Vec<crate::core::events::GameEvent> =
                            eq.iter().cloned().collect();
                        ...
                        {
                            for event in events {
                                history.record(current_turn, event);
                            }
                        }
                    }
                }
                ...
                if let Some(mut eq) = self.resources.get_mut::<crate::core::events::EventQueue>() {
                    eq.clear(current_turn + 1);
                }
            }

            // GameState 동기화 복구
            if let Some(st) = self.resources.get::<GameState>() {
                self.game_state = (*st).clone();
            }

            //
            if let Some(mut log) = self.resources.get_mut::<crate::ui::log::GameLog>() {
                if log.needs_more {
                    self.game_state = GameState::More;
                    let turn = self.resources.get::<u64>().map(|t| *t).unwrap_or(0);
            ...
            if let Some(change) = level_change {
                ...
                let next_level = match change {
                    LevelChange::NextLevel => LevelID::new(
                        self.dungeon.current_level.branch,
                        self.dungeon.current_level.depth + 1,
                    ),
                ...
                if next_level.depth >= 1 {
                    ...
                    // 1. 현재 그리드 저장
                    self.dungeon
                        .set_level(self.dungeon.current_level, self.grid.clone());

                    // 2. 새로운 층 로드 또는 생성
                    let (new_grid, stairs_pos) = if let Some(existing) =
                        self.dungeon.get_level(next_level)
                    {
                        ...
                        let monster_templates: Vec<_> =
                            self.assets.monsters.templates.values().collect();
                        let ltype =
                    ...
                    self.grid = new_grid;
                    self.dungeon.current_level = next_level;

                    //
                    let mut query =
                        <(Entity, &mut Position, &mut crate::core::entity::Level)>::query()
                            .filter(component::<PlayerTag>());
                    for (_, pos, level) in query.iter_mut(&mut self.world) {
                        pos.x = stairs_pos.0;
                        pos.y = stairs_pos.1;
                        level.0 = next_level;
                    ...
                    // 4. 리소스 갱신
                    self.resources.insert(self.grid.clone());
                    self.resources.insert(self.dungeon.clone());
                }
            ...
            let mut is_dead = false;
            {
                ...
                if let Some(h) = q.iter(&self.world).next() {
                    if h.current <= 0 {
                        is_dead = true;
                    }
                }
