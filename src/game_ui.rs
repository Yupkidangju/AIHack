    ...
    pub(crate) fn render_game_ui(&mut self, ctx: &egui::Context) {
        //
        {
            let player_info: Option<(Entity, i32, i32)> = {
                ...
                q.iter(&self.world).next().map(|(e, p)| (*e, p.x, p.y))
            };
        ...
        // (A) 턴 정보 추출
        let turn_count = self.resources.get::<u64>().map(|t| *t).unwrap_or(0);

        // (B) 메뉴 바 렌더링 (최상단)
        {
            use crate::ui::layout::menu_bar::{render_menu_bar, MenuAction};
            let menu_action = render_menu_bar(ctx, turn_count, &mut self.layout_settings);
            match menu_action {
                MenuAction::Inventory => {
                    self.last_cmd = Command::Inventory;
                }
        ...
        // (C) 커맨드 바 렌더링 (최하단)
        {
            ...
            let cmd_action = render_command_bar(ctx, self.layout_settings.command_advanced_mode);
            // 커맨드 바 클릭 → 기존 Command enum으로 변환
            match cmd_action {
                CommandBarAction::Pickup => {
                    self.last_cmd = Command::Pickup;
                }
                CommandBarAction::Inventory => {
                    self.last_cmd = Command::Inventory;
                }
                CommandBarAction::Eat => {
                    self.last_cmd = Command::Eat;
                }
                CommandBarAction::Quaff => {
                    self.last_cmd = Command::Quaff;
                }
                CommandBarAction::Read => {
                    self.last_cmd = Command::Read;
                }
                CommandBarAction::Apply => {
                    self.last_cmd = Command::Apply;
                }
                CommandBarAction::Zap => {
                    self.last_cmd = Command::Zap;
                }
                CommandBarAction::Cast => {
                    self.last_cmd = Command::Cast;
                }
                CommandBarAction::Pray => {
                    self.last_cmd = Command::Pray;
                }
                CommandBarAction::Search => {
                    self.last_cmd = Command::Search;
                }
                CommandBarAction::Help => {
                    self.last_cmd = Command::Help;
                }
                CommandBarAction::Throw => {
                    self.last_cmd = Command::Throw;
                }
                CommandBarAction::Kick => {
                    self.last_cmd = Command::Kick;
                }
                CommandBarAction::Open => {
                    self.last_cmd = Command::Open;
                }
                CommandBarAction::Close => {
                    self.last_cmd = Command::Close;
                }
                CommandBarAction::Wear => {
                    self.last_cmd = Command::Wear;
                }
                CommandBarAction::TakeOff => {
                    self.last_cmd = Command::TakeOff;
                }
                CommandBarAction::Wield => {
                    self.last_cmd = Command::Wield;
                }
                CommandBarAction::Engrave => {
                    self.last_cmd = Command::Engrave;
                }
                CommandBarAction::Name => {
                    self.last_cmd = Command::Name;
                }
        ...
        // (D) 상태 바 렌더링 (커맨드 바 바로 위)
        {
            ...
            if let Some((player, health, stats, status)) = query.iter(&self.world).next() {
                ...
                // 던전 깊이 문자열
                status_info.depth_str = match self.dungeon.current_level.branch {
                    crate::core::dungeon::DungeonBranch::Main => {
                        format!("Dlvl:{}", self.dungeon.current_level.depth)
                    }
                    crate::core::dungeon::DungeonBranch::Mines => {
                        format!("Mines:{}", self.dungeon.current_level.depth)
                    }
                    crate::core::dungeon::DungeonBranch::Sokoban => {
                        format!("Sokoban:{}", self.dungeon.current_level.depth)
                    }
                    _ => format!("Dlvl:{}", self.dungeon.current_level.depth),
                };
        ...
        // (E) 우측 스탯 패널 (조건부 표시)
        if self.layout_settings.show_stats_panel {
            ...
            if let Some((player, health, stats)) = query.iter(&self.world).next() {
                panel_data.name = if self.char_name_buf.is_empty() {
                    format!("{:?}", player.role)
                } else {
                    self.char_name_buf.clone()
                };
                ...
                panel_data.depth = match self.dungeon.current_level.branch {
                    crate::core::dungeon::DungeonBranch::Main => {
                        format!("Dlvl:{}", self.dungeon.current_level.depth)
                    }
                    _ => format!(
                        "{:?}:{}",
                        self.dungeon.current_level.branch, self.dungeon.current_level.depth
                    ),
                };
                ...
                panel_data.equipment = EquipmentSummary::default();
                {
                    ...
                    if let Some((equip,)) = eq_query.iter(&self.world).next() {
                        ...
                        for (slot, mut setter) in slot_mapping {
                            if let Some(&item_ent) = equip.slots.get(&slot) {
                                if let Ok(entry) = self.world.entry_ref(item_ent) {
                                    if let Ok(item) =
        ...
        //
        if self.layout_settings.show_minimap || self.layout_settings.show_message_panel {
            egui::SidePanel::left("left_panel")
                ...
                .show(ctx, |ui| {
                    // 미니맵
                    if self.layout_settings.show_minimap {
                        //
                        let mut px = 0usize;
                        let mut py = 0usize;
                        {
                            let mut pq = <&Position>::query().filter(component::<PlayerTag>());
                            if let Some(pos) = pq.iter(&self.world).next() {
                                px = pos.x as usize;
                                py = pos.y as usize;
                            }
                        }
                    ...
                    // 메시지 로그
                    if self.layout_settings.show_message_panel {
                        if let Some(log) = self.resources.get::<crate::ui::log::GameLog>() {
        ...
        egui::CentralPanel::default().show(ctx, |ui| {
            ui.horizontal(|ui| {
                ...
                let current_set = &self.assets.symbols.current_set;
            ...
            {
                self.renderer
                    .render_frame(&self.grid, &self.world, &self.assets, &vision);
            }

            // Ratatui 버퍼 내용을 egui Painter로 복사
            let buffer = self.renderer.terminal.backend().buffer();
            ...
            egui::Window::new("Message History (Ctrl+P)")
                .open(&mut self.show_log_history)
                .show(ctx, |ui| {
                        ...
                        && gy < crate::core::dungeon::ROWNO as i32
                    {
                        ...
                        if is_visible || is_memorized {
                            egui::show_tooltip(ctx, egui::Id::new("map_tooltip"), |ui| {
                                ...
                                // 타일 정보
                                if let Some(tile) = self.grid.get_tile(gx as usize, gy as usize) {
                                ...
                                if is_visible {
                                    ...
                                    for (_ent, pos, item) in i_query.iter(&self.world) {
                                    ...
                                    for (_ent, pos, monster) in m_query.iter(&self.world) {
                                        if pos.x == gx && pos.y == gy {
                                            ui.colored_label(
                                                egui::Color32::LIGHT_RED,
                        ...
                        if let Some((gx, gy)) = mouse::screen_to_grid(
                            ...
                            match action {
                                mouse::MouseAction::AdjacentMove { dir, .. } => {
                                    // 인접 이동/공격 (1턴)
                                    self.last_cmd = mouse::direction_to_command(dir);
                                    // Travel 모드 해제
                                ...
                                mouse::MouseAction::Travel { target_x, target_y } => {
                                    ...
                                    if let Some(mut p) = path {
                                        if p.len() > 1 {
                                            //
                                            self.travel_path = p.split_off(1);
                                        }
                                    }
                                }
                ...
                if response.secondary_clicked() {
                    if let Some(click_pos) = response.interact_pointer_pos() {
                        if let Some((gx, gy)) = mouse::screen_to_grid(
                            ...
                            self.context_menu_state.visible = true;
                            self.context_menu_state.grid_x = gx;
                            self.context_menu_state.grid_y = gy;
                        }
                    }
                }
            }
        ...
        // ================================================================
        if self.layout_settings.show_settings {
            let mut open = self.layout_settings.show_settings;
            egui::Window::new("? Settings")
                ...
                .show(ctx, |ui| {
                    ...
                    // 자동 줍기 설정
                    ui.checkbox(&mut self.options.autopickup, "Autopickup (자동 줍기)");
                    if self.options.autopickup {
                        ui.horizontal(|ui| {
                            ui.label("  Pickup types:");
                            ui.text_edit_singleline(&mut self.options.pickup_types);
                        });
                    }
                    ...
                    // 표시 옵션
                    ui.checkbox(&mut self.options.show_exp, "Show Experience (경험치 표시)");
                    ui.checkbox(&mut self.options.show_score, "Show Score (점수 표시)");
                    ui.checkbox(&mut self.options.color, "Use Colors (색상 사용)");
                    ui.checkbox(&mut self.options.hilite_pet, "Highlight Pets (펫 강조)");
                    ...
                    for set in &symbol_sets {
                        let selected = self.options.current_symbol_set == *set;
                        if ui.selectable_label(selected, *set).clicked() {
                            self.options.current_symbol_set = set.to_string();
                            self.assets.symbols.current_set = set.to_string();
                        }
                    }
                    ...
                    ui.horizontal(|ui| {
                        ui.label("Dog name:");
                        ui.text_edit_singleline(&mut self.options.dogname);
                    });
                    ui.horizontal(|ui| {
                        ui.label("Cat name:");
                        ui.text_edit_singleline(&mut self.options.catname);
                    });
                    ...
                    if ui.button("?? Save Options").clicked() {
                        self.options.save();
                        if let Some(mut log) = self.resources.get_mut::<crate::ui::log::GameLog>() {
                            let turn = self.resources.get::<u64>().map(|t| *t).unwrap_or(0);
                            log.add("Options saved.", turn);
                        }
                    }
                });
            self.layout_settings.show_settings = open;
        }

        // ================================================================
        // [v1.9.0 M4] 확장 명령(#) 입력 팝업
        // ================================================================
        if self.ext_cmd_mode {
            ...
            egui::Window::new("Extended Command")
                ...
                .show(ctx, |ui| {
                    ui.label("# 명령어를 입력하세요:");
                    let re = ui.text_edit_singleline(&mut self.ext_cmd_input);
                    re.request_focus();

                    // Enter 키로 확인
                    if ui.input(|i| i.key_pressed(egui::Key::Enter)) {
                        if let Some(cmd) = Command::from_extended_str(&self.ext_cmd_input) {
                            resolved_cmd = cmd;
                        }
                        close_popup = true;
                    }
                    ...
                    // 자동 완성 후보 목록 표시
                    if !self.ext_cmd_input.is_empty() {
                        let input_lower = self.ext_cmd_input.to_lowercase();
                        let candidates = [
                            "adjust",
                            "chat",
                            "conduct",
                        ...
                        if !matches.is_empty() {
                            ...
                            for m in &matches {
                                if ui.selectable_label(false, **m).clicked() {
                                    self.ext_cmd_input = m.to_string();
                                    if let Some(cmd) =
                                        Command::from_extended_str(&self.ext_cmd_input)
                                    {
                                        resolved_cmd = cmd;
                                    }
                                    close_popup = true;
        ...
        // [M4] 확장 명령 진입
        if self.last_cmd == Command::ExtendedCommand && !self.ext_cmd_mode {
            self.ext_cmd_mode = true;
            self.ext_cmd_input.clear();
            self.last_cmd = Command::Unknown; // 이번 프레임은 명령 없음
        }
        ...
        match self.last_cmd {
            Command::RunN => {
                self.run_direction = Some(crate::core::game_state::Direction::North);
                self.last_cmd = Command::MoveN;
            }
            Command::RunS => {
                self.run_direction = Some(crate::core::game_state::Direction::South);
                self.last_cmd = Command::MoveS;
            }
            Command::RunE => {
                self.run_direction = Some(crate::core::game_state::Direction::East);
                self.last_cmd = Command::MoveE;
            }
            Command::RunW => {
                self.run_direction = Some(crate::core::game_state::Direction::West);
                self.last_cmd = Command::MoveW;
            }
            Command::RunNE => {
                self.run_direction = Some(crate::core::game_state::Direction::NorthEast);
                self.last_cmd = Command::MoveNE;
            }
            Command::RunNW => {
                self.run_direction = Some(crate::core::game_state::Direction::NorthWest);
                self.last_cmd = Command::MoveNW;
            }
            Command::RunSE => {
                self.run_direction = Some(crate::core::game_state::Direction::SouthEast);
                self.last_cmd = Command::MoveSE;
            }
            Command::RunSW => {
                self.run_direction = Some(crate::core::game_state::Direction::SouthWest);
                self.last_cmd = Command::MoveSW;
            }
            _ => {}
        }

        //
        if self.last_cmd == Command::Unknown {
            if let Some(dir) = self.run_direction {
                ...
                let player_pos: Option<(i32, i32)> = {
                    let mut pq = <&Position>::query().filter(component::<PlayerTag>());
                    pq.iter(&self.world).next().map(|p| (p.x, p.y))
                };
                ...
                if let Some((px, py)) = player_pos {
                    ...
                    // [v1.9.0] 음수 좌표 안전 가드
                    if nx >= 0 && ny >= 0 {
                        if let Some(tile) = self.grid.get_tile(nx as usize, ny as usize) {
                            use crate::core::dungeon::tile::TileType;
                ...
                if can_continue {
                        ...
                        .get::<crate::core::systems::vision::VisionSystem>()
                    {
                        ...
                        for mpos in mq.iter(&self.world) {
                            if mpos.x >= 0
                ...
                if can_continue {
                    self.last_cmd = crate::ui::mouse::direction_to_command(dir);
                } else {
                    self.run_direction = None; // 달리기 종료
                }
            }
        } else if self.last_cmd != Command::Unknown {
            //
            if self.run_direction.is_some()
                && !matches!(
                    self.last_cmd,
                    Command::MoveN
                        | Command::MoveS
        ...
        // ================================================================
        {
            let msg: Option<&str> = match self.last_cmd {
            ...
            // TurnCount 특별 처리
            if self.last_cmd == Command::TurnCount {
                let turn = self.resources.get::<u64>().map(|t| *t).unwrap_or(0);
        ...
        if !self.travel_path.is_empty() && self.last_cmd == Command::Unknown {
            let (tx, ty) = self.travel_path[0];
            //
            let mut px = 0i32;
            let mut py = 0i32;
            {
        ...
        egui::Window::new("Character Status (C)")
            ...
            .show(ctx, |ui| {
                ...
                .filter(component::<PlayerTag>());
                for (player, health, stats) in query.iter(&self.world) {
                    ui.horizontal(|ui| {
                        ui.label("Name: Hero");
                        ui.label(format!(
                            "St:{} Dx:{} Co:{} In:{} Wi:{} Ch:{}",
                    ...
                    let dlvl_str = match self.dungeon.current_level.branch {
                        crate::core::dungeon::DungeonBranch::Main => {
                            format!("Dlvl:{}", self.dungeon.current_level.depth)
                        }
                        crate::core::dungeon::DungeonBranch::Mines => {
        ...
        if self.game_state == GameState::Inventory
            || matches!(self.game_state, GameState::Looting { .. })
        {
            egui::Window::new("Inventory (i)").show(ctx, |ui| {
                ...
                .filter(component::<PlayerTag>());
                for (inv, equip) in query.iter(&self.world) {
                    let looting_container =
                        if let GameState::Looting { container } = self.game_state {
                            Some(container)
                        } else {
                            None
                        };
                    ...
                    if let Some(action) = action {
                        self.resources.insert(Some(action));
                    }
                }
            });
        }
        ...
        if let GameState::Looting { container } = self.game_state {
            egui::Window::new("Looting").show(ctx, |ui| {
                if let Ok(entry) = self.world.entry_ref(container) {
            ...
            egui::Window::new("Sacrifice Item").show(ctx, |ui| {
                ...
                for (inv, _) in query.iter(&self.world) {
                    if let Some(action) = crate::ui::widgets::offer::show_offer_menu(
                        ...
                        self.resources.insert(Some(action));
                        self.game_state = GameState::Normal; // 액션 선택 시 자동 닫힘
                    }
                }
                    ...
                    if (edit.lost_focus() && ctx.input(|i| i.key_pressed(egui::Key::Enter))) || ui.button("Apply").clicked() {
                        ...
                        if let Some(target) = entity {
                            if is_call {
                                ...
                                // Name logic
                                if let Ok(mut entry) = self.world.entry_mut(target) {
                                ...
                                if let Some(n) = &name_to_set {
                                    ...
                                    let player_opt = p_query.iter(&self.world).next().cloned();
                                    if let Some(player) = player_opt {
                                        let mut log = self.resources.get_mut::<crate::ui::log::GameLog>().unwrap();
                                        let turn = self.resources.get::<u64>().map(|t| *t).unwrap_or(0);
        ...
        if self.game_state == GameState::SelectInvoke {
            egui::Window::new("Invoke Artifact").show(ctx, |ui| {
                ...
                if let Some(target) = selected {
                    ...
                    let player_ent = p_query.iter(&self.world).next().cloned();
        ...
        if self.game_state == GameState::ConfirmDrinkFountain {
            egui::Window::new("Drink").show(ctx, |ui| {
                ui.label("Drink from the fountain?");
                ui.horizontal(|ui| {
                    if ui.button("Yes").clicked() {
                        // 분수 마시기 실행
                        let (mut subworld, _) = self.world.split_for_query(&<(
                        ...
                        if let Some(mut log) = self.resources.get_mut::<crate::ui::log::GameLog>() {
                            let turn = self.resources.get::<u64>().map(|t| *t).unwrap_or(0);
                            if let Some(mut rng) =
                                self.resources.get_mut::<crate::util::rng::NetHackRng>()
                            {
        ...
        if self.game_state == GameState::SelectOffhand {
            let mut selected_action = None;
            egui::Window::new("Dual Wield").show(ctx, |ui| {
                let ident_table = self
                    .resources
                    .get::<crate::core::entity::identity::IdentityTable>()
                    .unwrap();
                let mut query =
        ...
        if self.game_state == GameState::SelectQuiver {
            let mut selected_action = None;
        ...
        if self.game_state == GameState::SelectEngraveTool {
        ...
        if let GameState::EngravingText { tool } = self.game_state {
            egui::Window::new("Engrave Text").show(ctx, |ui| {
                    ...
                    || ui.button("Write").clicked()
                {
                    ...
                    self.engraving_input.clear();
                    self.game_state = GameState::Normal;

                    //
                    if let Some(mut t) = self.resources.get_mut::<u64>() {
                        *t += 1;
                        log.current_turn = *t;
                    }
                }
        ...
        // Help Window (?)
        if self.game_state == GameState::Help {
        ...
        if self.layout_settings.show_settings {
            ...
            egui::Window::new("Settings")
                ...
                .show(ctx, |ui| {
                    ui.label("심볼 세트 선택:");
                    for set_name in self.assets.symbols.sets.keys() {
                        if ui
                            .selectable_label(
                                &self.assets.symbols.current_set == set_name,
                                set_name,
                            )
                            .clicked()
                        {
                            self.assets.symbols.current_set = set_name.clone();
                            self.options.current_symbol_set = set_name.clone();
                            self.options.save();
                    ...
                    ui.separator();
                    ui.checkbox(&mut self.options.autopickup, "Autopickup");
        ...
        // 5. Game Over Overlay
        let game_over_info = if let GameState::GameOver { message } = &self.game_state {
            Some(message.clone())
        } else {
            None
        };
                    ...
                    ui.vertical_centered(|ui| {
                        ...
                        let gold = 0; // TODO: Fetch from Player
                        {
                            let mut q = <&crate::core::entity::player::Player>::query()
                                .filter(component::<PlayerTag>());
                            if let Some(p) = q.iter(&self.world).next() {
                                xp = p.experience;
                            }
                        }
