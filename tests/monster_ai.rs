...
fn current_level_hostile_monsters_are_filtered_and_sorted() {
    ...
    let level2_monster = aihack::testing::SessionBuilder::mutate(&mut session, |world| {
        world
            .saved()
            .entities
    ...
    assert!(aihack::testing::SessionBuilder::mutate(
        &mut session,
        |world| world.saved().entities.set_actor_location(
            level2_monster,
            LevelId::main(2),
            Pos { x: 7, y: 7 }
        )
    ...
    aihack::testing::SessionBuilder::mutate(&mut session, |world| {
        world.set_player_location(LevelId::main(2), Pos { x: 5, y: 5 });
    });
...
fn monster_turn_order_is_entity_id_sorted() {
    ...
    let attackers = outcome
        ...
        .filter_map(|event| match event {
            GameEvent::AttackResolved { attacker, .. }
                if *attacker != session.world().player_id() =>
            {
                Some(*attacker)
            }
            _ => None,
...
fn floating_eye_does_not_attack_when_adjacent() {
    ...
    kill_fixture_monsters(&mut session);
    aihack::testing::SessionBuilder::mutate(&mut session, |world| {
        world.set_player_pos(Pos { x: 8, y: 8 })
    });
    let eye = aihack::testing::SessionBuilder::mutate(&mut session, |world| {
        world
            .saved()
...
fn movement_events_include_actor_identity() {
    ...
    aihack::testing::SessionBuilder::mutate(&mut player_session, |world| {
    ...
    aihack::testing::SessionBuilder::mutate(&mut monster_session, |world| {
        world.saved().entities.set_alive(EntityId(2), false)
    });
    aihack::testing::SessionBuilder::mutate(&mut monster_session, |world| {
        world.set_player_pos(Pos { x: 17, y: 12 })
